#!/usr/bin/env python3
"""
批量修复core/framework/agents目录下的类型注解错误

常见错误模式：
1. Optional[type[BaseAgent]]] → Optional[type[BaseAgent]]
2. -> dict[str, Any]]: → -> dict[str, Any]:
3. Optional[list[str] = field( → Optional[list[str]] = field(
"""
...
def main():
    ...
    for file_path in py_files:
        ...
        if success:
            success_count += 1
            if fixes > 0:
                print(f"✅ {file_path.relative_to(base_dir)}: 修复 {fixes} 处")
                total_fixes += fixes
        else:
            error_count += 1
