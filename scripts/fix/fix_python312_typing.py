...
# 错误的模式: Optional[Type] = None
ERROR_PATTERN = re.compile(r':\s*([A-Z][a-zA-Z0-9_.\[\] ]*)\]\s*=\s*None\)')
ERROR_PATTERN2 = re.compile(r':\s*([A-Z][a-zA-Z0-9_.\[\] ]*)\]\s*=\s*None')
...
def fix_typing_in_file(file_path: Path) -> tuple[bool, int, str]:
    ...
    for match in ERROR_PATTERN.finditer(content):
        ...
        content = content.replace(error_text, fixed_text)
        fixes.append(f"修复: {error_text} -> {fixed_text}")
        fix_count += 1

    # 第二种模式: 不带括号的
    for match in ERROR_PATTERN2.finditer(content):
        error_text = match.group(0)
        type_name = match.group(1)
        # 确保不是已经修复过的
        if not error_text.endswith(')'):
            fixed_text = f': Optional[{type_name}] = None'
            if fixed_text != error_text:
                content = content.replace(error_text, fixed_text)
                fixes.append(f"修复: {error_text} -> {fixed_text}")
                fix_count += 1

    # 2. 检查是否需要添加Optional导入
    ...
    for py_file in root_dir.rglob('*.py'):
        ...
        if not dry_run:
            ...
            if modified:
                ...
                print(f"✅ 修复: {py_file.relative_to(root_dir)} ({fix_count}处) - {detail}")
            elif '失败' in detail:
                stats['error_files'] += 1
