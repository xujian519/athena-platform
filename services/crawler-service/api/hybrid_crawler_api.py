...
from core.batch_processor import TaskPriority, batch_processor, batch_processor_context
from core.hybrid_crawler_manager import CrawlerType, HybridCrawlerManager
...
class HybridCrawlRequest(BaseModel):
    ...
    # 高级配置
    crawl4ai_mode: Optional[str] = Field('basic', description='Crawl4AI模式')
    firecrawl_mode: Optional[str] = Field('scrape', description='FireCrawl模式')
    llm_prompt: Optional[str] = Field(None, description='LLM提取提示')

class CrawlerConfigUpdate(BaseModel):
    """爬虫配置更新模型"""
    cost_limits: Optional[Dict[str, Any]] = Field(None, description='成本限制配置')
    routing: Optional[Dict[str, Any]] = Field(None, description='路由配置')
    crawl4ai: Optional[Dict[str, Any]] = Field(None, description='Crawl4AI配置')
    firecrawl: Optional[Dict[str, Any]] = Field(None, description='FireCrawl配置')
    internal: Optional[Dict[str, Any]] = Field(None, description='内部爬虫配置')
...
async def list_tasks(
    status: Optional[str] = Query(None, description='按状态过滤: running, completed, failed'),
...
async def get_batch_stats():
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取批量统计失败: {str(e)}")
