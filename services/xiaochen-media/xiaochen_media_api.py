...
# 获取小宸身份信息
xiaochen_identity = identity_manager.get_agent_identity("xiaochen")
if not xiaochen_identity:
    # 使用默认身份
    xiaochen_identity = {
        "name": "小宸",
        "full_name": "小宸·射手座",
...
async def root():
    """主页 - 展示身份信息"""
    service_info = identity_manager.get_service_info("xiaochen") or {
...
async def generate_content(request: ContentRequest):
    ...
    return {
        ...
        "content_ideas": [
            {
                "title": f"{request.topic}深度解析",
                "hook": "3分钟让你彻底理解",
                "structure": ["痛点引入", "解决方案", "实操演示", "总结升华"],
                "estimated_engagement": "85%"
            },
            {
                "title": f"{request.topic}避坑指南",
                "hook": "99%的人都不知道的细节",
                "structure": ["常见错误", "正确做法", "案例分析", "最佳实践"],
                "estimated_engagement": "92%"
            }
        ],
        "platform_tips": f"在{request.platform}平台，建议在晚间8-10点发布，配合热门话题标签",
        "slogan": xiaochen_identity["slogan"]
    }
