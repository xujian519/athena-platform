...
class QuickDecisionRequest(BaseModel):
    ...
    options: Optional[List[str]] = Field(None, description="快速选项列表")

class DecisionResponse(BaseModel):
    success: bool = Field(..., description="决策是否成功")
    chosen_option: Optional[str] = Field(None, description="选择的选项ID")
    confidence: float = Field(..., description="决策置信度")
    human_feedback: Optional[str] = Field(None, description="人类反馈")
    rationale: Optional[str] = Field(None, description="决策理由")
...
async def quick_decision(request: QuickDecisionRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"快速决策失败: {str(e)}")
...
async def batch_decision(requests: List[DecisionRequest]):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"批量决策失败: {str(e)}")
