...
def main():
    ...
    try:
        if manager.deploy_all():
            ...
            logger.info(f"\n💡 使用说明:")
            logger.info(f"   1. next-ai-draw-io: http://localhost:8081")
            logger.info(f"   2. SketchAgent: http://localhost:8080/health")
            logger.info(f"   3. 请确保在 next-ai-draw-io/.env 中设置您的OpenAI API Key")
            logger.info(f"   4. 集成到专利系统: 修改 patent_ai_drawing_integration.py 中的服务地址")
