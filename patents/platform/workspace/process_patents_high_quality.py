    ...
    def extract_entities(self, text: str, source_file: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        for entity_type, pattern in self.legal_entities_patterns.items():
            ...
            for match in matches:
                ...
                entity_id = hashlib.md5(f"{entity_value}_{entity_type}".encode('utf-8'), usedforsecurity=False).hexdigest()[:16]
        ...
        for category, terms in self.quality_entities.items():
            for term in terms:
                if term in text:
                    entity_id = hashlib.md5(f"{term}_{category}".encode('utf-8'), usedforsecurity=False).hexdigest()[:16]
                    entity = {
                        'id': entity_id,
                        'type': category,
                        'value': term,
...
class QualityController:
    ...
    def assess_quality(self, entities: List[Dict], relations: List[Dict]) -> Dict:
        ...
        # 多维度质量评分
        diversity_score = len(set(e['type'] for e in entities)) / 10.0  # 实体类型多样性
...
def main():
    ...
    try:
        ...
        # 显示质量改进说明
        logger.info(f"\n💡 质量改进特性:")
        logger.info(f"   ✅ 自动编码检测和处理")
        logger.info(f"   ✅ 多维度质量评估")
        logger.info(f"   ✅ 预定义高质量实体库")
        logger.info(f"   ✅ 智能关系推断")
        logger.info(f"   ✅ 详细的处理日志")
