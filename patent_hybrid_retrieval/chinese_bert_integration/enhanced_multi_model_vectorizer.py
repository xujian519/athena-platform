#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
增强版多模型向量融合器
Enhanced Multi-Model Vectorizer

集成Redis缓存和并行处理优化
"""

import asyncio
import logging
import multiprocessing as mp
import os
import pickle
from collections import defaultdict
from concurrent.futures import ProcessPoolExecutor, ThreadPoolExecutor, as_completed
from datetime import datetime
from functools import partial
from typing import Any, Dict, List, Optional, Tuple, Union

import numpy as np
import torch
from intelligent_model_selector import IntelligentModelSelector
from model_manager import ChineseBERTModelManager
from redis_cache_manager import RedisCacheManager

# 配置日志
logger = logging.getLogger(__name__)

class EnhancedMultiModelVectorizer:
    """增强版多模型向量融合器"""

    def __init__(
        self,
        cache_dir: str = '/Users/xujian/Athena工作平台/patent_hybrid_retrieval/cache',
        max_cache_size: int = 10000,
        enable_redis: bool = True,
        redis_config: Optional[Dict[str, Any]] = None,
        max_workers: int = None,
        enable_parallel: bool = True
    ):
        """初始化增强版多模型向量器

        Args:
            cache_dir: 本地缓存目录
            max_cache_size: 最大缓存条目数
            enable_redis: 是否启用Redis缓存
            redis_config: Redis配置
            max_workers: 最大并行工作数
            enable_parallel: 是否启用并行处理
        """
        self.model_manager = ChineseBERTModelManager()
        self.selector = IntelligentModelSelector()
        self.cache_dir = cache_dir
        self.max_cache_size = max_cache_size
        self.enable_redis = enable_redis
        self.enable_parallel = enable_parallel

        # 初始化缓存管理器
        if enable_redis:
            self.cache_manager = RedisCacheManager()
        else:
            self.cache_manager = None

        # 本地缓存（作为备用）
        self.vector_cache = {}

        # 并行处理配置
        if max_workers is None:
            self.max_workers = min(4, mp.cpu_count())
        else:
            self.max_workers = max_workers

        # 融合策略
        self.fusion_strategies = {
            'concatenation': self._concatenation_fusion,
            'weighted_average': self._weighted_average_fusion,
            'attention_fusion': self._attention_fusion,
            'adaptive_fusion': self._adaptive_fusion
        }

        # 性能统计
        self.stats = {
            'total_requests': 0,
            'cache_hits': 0,
            'model_switches': defaultdict(int),
            'fusion_times': defaultdict(list),
            'parallel_jobs': 0,
            'avg_parallel_speedup': 1.0,
            'last_updated': datetime.now()
        }

    def encode_with_multiple_models_enhanced(
        self,
        texts: List[str],
        model_config: Optional[Dict[str, Any]] = None,
        fusion_strategy: str = 'weighted_average',
        return_individual: bool = False,
        use_cache: bool = True,
        parallel_threshold: int = 10
    ) -> Dict[str, Any]:
        """使用多个模型编码文本（增强版）

        Args:
            texts: 文本列表
            model_config: 模型配置
            fusion_strategy: 融合策略
            return_individual: 是否返回各模型的独立结果
            use_cache: 是否使用缓存
            parallel_threshold: 并行处理阈值（文本数量超过此值时启用并行）

        Returns:
            编码结果字典
        """
        start_time = datetime.now()
        self.stats['total_requests'] += 1

        # 检查缓存
        if use_cache and self.cache_manager:
            cache_key = self._get_cache_key(texts, model_config, fusion_strategy)
            cached_result = self._get_from_cache(cache_key)
            if cached_result:
                cached_result['from_cache'] = True
                return cached_result

        # 确定使用的模型
        if not model_config:
            model_config = {
                'auto_select': True,
                'max_models': 3
            }

        model_names = self._determine_models(texts, model_config)

        # 获取权重
        weights = model_config.get('weights', [1.0] * len(model_names))
        weights = self._normalize_weights(weights)

        # 决定是否使用并行处理
        use_parallel = self.enable_parallel and len(texts) >= parallel_threshold

        # 多模型编码
        embeddings = {}
        dimensions = set()

        if use_parallel:
            embeddings = self._parallel_encode(texts, model_names)
        else:
            embeddings = self._sequential_encode(texts, model_names)

        dimensions.update(v.shape[1] for v in embeddings.values() if v is not None)

        # 检查是否有成功的编码
        if not embeddings:
            logger.error('没有模型成功编码')
            return {'error': 'no_successful_encodings'}

        # 融合向量
        fusion_start = datetime.now()
        fusion_func = self.fusion_strategies.get(fusion_strategy, self._weighted_average_fusion)

        try:
            fused_vector, fusion_info = fusion_func(embeddings, weights)
        except Exception as e:
            logger.error(f"向量融合失败: {e}")
            fused_vector, fusion_info = self._simple_average_fusion(embeddings, weights)

        fusion_time = (datetime.now() - fusion_start).total_seconds()
        self.stats['fusion_times'][fusion_strategy].append(fusion_time)

        # 准备结果
        result = {
            'fused_vector': fused_vector,
            'model_info': self._get_model_info(model_names),
            'fusion_info': {
                'strategy': fusion_strategy,
                'models_used': list(embeddings.keys()),
                'weights': weights,
                'dimensions': list(dimensions),
                'fusion_time': fusion_time,
                'details': fusion_info,
                'parallel_used': use_parallel,
                'parallel_workers': self.max_workers if use_parallel else 1
            },
            'from_cache': False
        }

        if return_individual:
            result['individual_vectors'] = embeddings

        # 缓存结果
        if use_cache and self.cache_manager:
            self._cache_result(cache_key, result)

        # 更新统计
        total_time = (datetime.now() - start_time).total_seconds()
        if use_parallel:
            self.stats['parallel_jobs'] += 1
            # 计算并行加速比（简化计算）
            if fusion_time > 0:
                speedup = len(texts) * len(model_names) / (fusion_time * self.max_workers)
                self.stats['avg_parallel_speedup'] = (
                    (self.stats['avg_parallel_speedup'] + speedup) / 2
                )

        logger.info(f"多模型编码完成: {len(texts)} 个文本, {len(embeddings)} 个模型, "
                   f"并行={'是' if use_parallel else '否'}, 耗时 {total_time:.3f}s")

        return result

    def _determine_models(self, texts: List[str], model_config: Dict[str, Any]) -> List[str]:
        """确定要使用的模型"""
        model_names = model_config.get('models', [])

        if not model_names or model_config.get('auto_select', False):
            if texts:
                # 使用第一个查询文本进行模型选择
                model_names = [name for name, _, _ in self.selector.select_models_for_ensemble(
                    texts[0], model_config.get('max_models', 3)
                )]
            else:
                model_names = ['bge-base-zh-v1.5']

        # 限制模型数量
        max_models = model_config.get('max_models', len(model_names))
        model_names = model_names[:max_models]

        return model_names

    def _parallel_encode(self, texts: List[str], model_names: List[str]) -> Dict[str, np.ndarray]:
        """并行编码文本"""
        embeddings = {}

        def encode_with_model_parallel(model_name):
            """单个模型并行编码"""
            if not self.model_manager.load_model(model_name):
                return None

            try:
                # 将文本分成批次进行并行处理
                batch_size = max(1, len(texts) // self.max_workers)
                batches = [texts[i:i+batch_size] for i in range(0, len(texts), batch_size)]

                batch_embeddings = []
                for batch in batches:
                    vectors = self.model_manager.encode_texts(batch, model_name=model_name)
                    if vectors is not None:
                        batch_embeddings.append(vectors)

                if batch_embeddings:
                    # 合并所有批次的结果
                    return model_name, np.vstack(batch_embeddings)
                else:
                    return None

            except Exception as e:
                logger.error(f"模型 {model_name} 并行编码失败: {e}")
                return None

        # 使用线程池并行执行多个模型
        with ThreadPoolExecutor(max_workers=min(self.max_workers, len(model_names))) as executor:
            futures = {executor.submit(encode_with_model_parallel, name): name for name in model_names}

            for future in as_completed(futures):
                result = future.result()
                if result:
                    model_name, vectors = result
                    embeddings[model_name] = vectors
                    self.stats['model_switches'][model_name] += 1

        return embeddings

    def _sequential_encode(self, texts: List[str], model_names: List[str]) -> Dict[str, np.ndarray]:
        """顺序编码文本"""
        embeddings = {}

        for model_name in model_names:
            if self.model_manager.load_model(model_name):
                try:
                    vectors = self.model_manager.encode_texts(texts, model_name=model_name)
                    if vectors is not None:
                        embeddings[model_name] = vectors
                        self.stats['model_switches'][model_name] += 1
                except Exception as e:
                    logger.error(f"模型 {model_name} 编码失败: {e}")

        return embeddings

    def batch_encode_with_parallel(
        self,
        text_batches: List[List[str]],
        model_config: Optional[Dict[str, Any]] = None,
        fusion_strategy: str = 'weighted_average',
        parallel_batches: bool = True
    ) -> List[Dict[str, Any]]:
        """批量编码（支持批次级并行）

        Args:
            text_batches: 文本批次列表
            model_config: 模型配置
            fusion_strategy: 融合策略
            parallel_batches: 是否并行处理批次

        Returns:
            每个批次的编码结果
        """
        results = []

        if parallel_batches and len(text_batches) > 1:
            # 并行处理多个批次
            encode_func = partial(
                self._encode_single_batch,
                model_config=model_config,
                fusion_strategy=fusion_strategy
            )

            with ThreadPoolExecutor(max_workers=min(self.max_workers, len(text_batches))) as executor:
                futures = {executor.submit(encode_func, batch): i for i, batch in enumerate(text_batches)}
                results = [None] * len(text_batches)

                for future in as_completed(futures):
                    batch_idx = futures[future]
                    try:
                        result = future.result()
                        results[batch_idx] = result
                    except Exception as e:
                        logger.error(f"批次 {batch_idx} 编码失败: {e}")
                        results[batch_idx] = {'error': str(e)}
        else:
            # 顺序处理
            for i, batch in enumerate(text_batches):
                result = self._encode_single_batch(batch, model_config, fusion_strategy)
                results.append(result)

        return results

    def _encode_single_batch(
        self,
        texts: List[str],
        model_config: Optional[Dict[str, Any]],
        fusion_strategy: str
    ) -> Dict[str, Any]:
        """编码单个批次"""
        logger.info(f"编码批次，文本数: {len(texts)}")
        result = self.encode_with_multiple_models(
            texts,
            model_config=model_config,
            fusion_strategy=fusion_strategy
        )
        return result

    def _get_from_cache(self, cache_key: str) -> Dict[str, Any | None]:
        """从缓存获取结果"""
        if self.cache_manager and self.cache_manager.is_connected():
            try:
                result = self.cache_manager.get('vector', cache_key)
                if result:
                    self.stats['cache_hits'] += 1
                    return result
            except Exception as e:
                logger.error(f"Redis缓存获取失败: {e}")

        # 回退到本地缓存
        if cache_key in self.vector_cache:
            self.stats['cache_hits'] += 1
            return self.vector_cache[cache_key]

        return None

    def _cache_result(self, cache_key: str, result: Dict[str, Any]):
        """缓存结果"""
        if self.cache_manager and self.cache_manager.is_connected():
            try:
                self.cache_manager.set('vector', cache_key, value=result, ttl=3600)
                return
            except Exception as e:
                logger.error(f"Redis缓存设置失败: {e}")

        # 回退到本地缓存
        if len(self.vector_cache) >= self.max_cache_size:
            # 简单的LRU策略：删除最旧的缓存项
            oldest_key = next(iter(self.vector_cache))
            del self.vector_cache[oldest_key]

        self.vector_cache[cache_key] = result

    def _get_cache_key(self, texts: List[str], config: Dict, strategy: str) -> str:
        """生成缓存键"""
        import hashlib
        content = {
            'texts': texts[:5],  # 只用前5个文本生成key
            'config': config,
            'strategy': strategy
        }
        content_str = str(content)
        return hashlib.md5(content_str.encode('utf-8'), usedforsecurity=False).hexdigest()

    def _normalize_weights(self, weights: List[float]) -> List[float]:
        """归一化权重"""
        total_weight = sum(weights)
        if total_weight == 0:
            return [1.0 / len(weights)] if weights else [1.0]
        return [w / total_weight for w in weights]

    def _get_model_info(self, model_names: List[str]) -> Dict[str, Any]:
        """获取模型信息"""
        info = {}
        for model_name in model_names:
            model_info = self.model_manager.get_model_info(model_name)
            if model_info:
                info[model_name] = model_info
        return info

    def _concatenation_fusion(
        self,
        embeddings: Dict[str, np.ndarray],
        weights: List[float]
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """拼接融合策略"""
        vectors = list(embeddings.values())
        max_length = max(v.shape[1] for v in vectors)

        # 填充到相同维度
        padded_vectors = []
        for v in vectors:
            if v.shape[1] < max_length:
                padding = np.zeros((v.shape[0], max_length - v.shape[1]))
                v_padded = np.hstack([v, padding])
            else:
                v_padded = v
            padded_vectors.append(v_padded)

        # 拼接
        fused = np.hstack(padded_vectors)

        fusion_info = {
            'method': 'concatenation',
            'input_dimensions': [v.shape[1] for v in vectors],
            'output_dimension': fused.shape[1],
            'padding_used': any(v.shape[1] < max_length for v in vectors)
        }

        return fused, fusion_info

    def _weighted_average_fusion(
        self,
        embeddings: Dict[str, np.ndarray],
        weights: List[float]
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """加权平均融合策略"""
        vectors = list(embeddings.values())
        weights_array = np.array(weights)

        # 检查维度一致性
        dimensions = [v.shape[1] for v in vectors]
        if len(set(dimensions)) > 1:
            logger.warning(f"模型输出维度不一致: {dimensions}，使用维度对齐")
            # 使用平均填充
            max_dim = max(dimensions)
            aligned_vectors = []
            for v in vectors:
                if v.shape[1] < max_dim:
                    padding = np.zeros((v.shape[0], max_dim - v.shape[1]))
                    v_aligned = np.hstack([v, padding])
                else:
                    v_aligned = v
                aligned_vectors.append(v_aligned)
            vectors = aligned_vectors

        # 加权平均
        fused = np.zeros_like(vectors[0])
        for i, (v, weight) in enumerate(zip(vectors, weights_array)):
            fused += weight * v

        fusion_info = {
            'method': 'weighted_average',
            'weights': weights,
            'input_dimensions': dimensions,
            'output_dimension': fused.shape[1]
        }

        return fused, fusion_info

    def _attention_fusion(
        self,
        embeddings: Dict[str, np.ndarray],
        weights: List[float]
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """注意力融合策略（简化版）"""
        # 将weights作为注意力权重
        return self._weighted_average_fusion(embeddings, weights)

    def _adaptive_fusion(
        self,
        embeddings: Dict[str, np.ndarray],
        weights: List[float]
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """自适应融合策略"""
        # 简化实现：根据模型性能调整权重
        performance_scores = self._get_model_performance(list(embeddings.keys()))
        adapted_weights = []

        for i, (model_name, original_weight) in enumerate(zip(embeddings.keys(), weights)):
            performance = performance_scores.get(model_name, 1.0)
            adapted_weight = original_weight * performance
            adapted_weights.append(adapted_weight)

        # 归一化
        total_weight = sum(adapted_weights)
        adapted_weights = [w / total_weight for w in adapted_weights]

        return self._weighted_average_fusion(embeddings, adapted_weights)

    def _simple_average_fusion(
        self,
        embeddings: Dict[str, np.ndarray],
        weights: List[float]
    ) -> Tuple[np.ndarray, Dict[str, Any]]:
        """简单平均融合（回退策略）"""
        vectors = list(embeddings.values())

        # 确保维度一致
        max_dim = max(v.shape[1] for v in vectors)
        aligned_vectors = []
        for v in vectors:
            if v.shape[1] < max_dim:
                padding = np.zeros((v.shape[0], max_dim - v.shape[1]))
                v_aligned = np.hstack([v, padding])
            else:
                v_aligned = v
            aligned_vectors.append(v_aligned)

        # 平均
        fused = np.mean(aligned_vectors, axis=0)

        fusion_info = {
            'method': 'simple_average',
            'input_dimensions': [v.shape[1] for v in vectors],
            'output_dimension': fused.shape[1]
        }

        return fused, fusion_info

    def _get_model_performance(self, model_names: List[str]) -> Dict[str, float]:
        """获取模型性能分数"""
        performance_map = {
            'bge-large-zh-v1.5': 1.2,
            'chinese-legal-electra': 1.1,
            'bge-base-zh-v1.5': 0.9
        }

        return {
            name: performance_map.get(name, 1.0)
            for name in model_names
        }

    def encode_patents(
        self,
        patents: List[Dict[str, Any]],
        text_fields: List[str] = ['title', 'abstract'],
        model_config: Optional[Dict[str, Any]] = None,
        fusion_strategy: str = 'weighted_average',
        parallel_threshold: int = 5
    ) -> Dict[str, Any]:
        """编码专利数据（增强版）

        Args:
            patents: 专利数据列表
            text_fields: 要编码的文本字段
            model_config: 模型配置
            fusion_strategy: 融合策略
            parallel_threshold: 并行处理阈值

        Returns:
            包含向量的专利数据
        """
        # 准备文本
        texts = []
        for patent in patents:
            text_parts = []
            for field in text_fields:
                value = patent.get(field, '')
                if value:
                    text_parts.append(f"{field}: {value}")
            texts.append(' | '.join(text_parts))

        # 编码
        result = self.encode_with_multiple_models_enhanced(
            texts,
            model_config=model_config,
            fusion_strategy=fusion_strategy,
            parallel_threshold=parallel_threshold
        )

        # 将向量添加回专利数据
        if 'fused_vector' in result:
            fused_vectors = result['fused_vector']
            for i, patent in enumerate(patents):
                if i < len(fused_vectors):
                    patent['vector'] = fused_vectors[i].tolist()
                    patent['vector_dimension'] = fused_vectors.shape[1]

        # 添加融合信息
        patents_with_vectors = {
            'patents': patents,
            'encoding_info': result,
            'text_fields': text_fields
        }

        return patents_with_vectors

    def get_enhanced_stats(self) -> Dict[str, Any]:
        """获取增强的性能统计"""
        base_stats = {
            'total_requests': self.stats['total_requests'],
            'cache_hits': self.stats['cache_hits'],
            'cache_hit_rate': self.stats['cache_hits'] / max(1, self.stats['total_requests']),
            'model_switches': dict(self.stats['model_switches']),
            'parallel_jobs': self.stats['parallel_jobs'],
            'avg_parallel_speedup': self.stats['avg_parallel_speedup'],
            'last_updated': self.stats['last_updated']
        }

        # 融合策略统计
        fusion_stats = {}
        for strategy, times in self.stats['fusion_times'].items():
            fusion_stats[strategy] = {
                'count': len(times),
                'avg_time': np.mean(times) if times else 0,
                'min_time': min(times) if times else 0,
                'max_time': max(times) if times else 0
            }
        base_stats['fusion_stats'] = fusion_stats

        # 缓存统计
        if self.cache_manager:
            cache_stats = self.cache_manager.get_stats()
            base_stats['redis_stats'] = cache_stats

        return base_stats

    def clear_cache(self):
        """清空所有缓存"""
        self.vector_cache.clear()
        if self.cache_manager:
            self.cache_manager.clear_all()
        logger.info('所有缓存已清空')

    def cleanup(self):
        """清理资源"""
        self.clear_cache()
        self.model_manager.cleanup()
        if self.cache_manager:
            self.cache_manager.cleanup()

# 测试函数
def test_enhanced_vectorizer():
    """测试增强版多模型向量器"""
    logger.info("\n=== 测试增强版多模型向量融合器 ===\n")

    vectorizer = EnhancedMultiModelVectorizer(
        enable_redis=True,
        enable_parallel=True
    )

    try:
        # 测试大量文本
        test_texts = [
            f"测试文本{i+1}：本发明涉及一种电池管理系统，包括状态监测模块。"
            for i in range(20)
        ]

        logger.info('1. 测试并行编码...')
        start_time = time.time()

        result = vectorizer.encode_with_multiple_models_enhanced(
            test_texts,
            model_config={
                'auto_select': True,
                'max_models': 2
            },
            fusion_strategy='weighted_average',
            parallel_threshold=10
        )

        elapsed_time = time.time() - start_time

        logger.info(f"   ✅ 并行编码成功，耗时: {elapsed_time:.3f}s")
        logger.info(f"   向量维度: {result['fused_vector'].shape}")
        logger.info(f"   并行处理: {'是' if result['fusion_info']['parallel_used'] else '否'}")
        logger.info(f"   并行工作数: {result['fusion_info']['parallel_workers']}")

        # 测试批量编码
        logger.info("\n2. 测试批量编码...")
        text_batches = [test_texts[i:i+5] for i in range(0, len(test_texts), 5)]

        batch_start = time.time()
        batch_results = vectorizer.batch_encode_with_parallel(
            text_batches,
            parallel_batches=True
        )
        batch_elapsed = time.time() - batch_start

        logger.info(f"   ✅ 批量编码成功，耗时: {batch_elapsed:.3f}s")
        logger.info(f"   批次数: {len(batch_results)}")

        # 性能统计
        logger.info("\n3. 增强性能统计:")
        stats = vectorizer.get_enhanced_stats()
        logger.info(f"   总请求数: {stats['total_requests']}")
        logger.info(f"   缓存命中率: {stats['cache_hit_rate']:.2%}")
        logger.info(f"   并行任务数: {stats['parallel_jobs']}")
        logger.info(f"   并行加速比: {stats['avg_parallel_speedup']:.2f}x")

        if 'redis_stats' in stats:
            redis_stats = stats['redis_stats']
            logger.info(f"   Redis连接: {'是' if redis_stats['connected'] else '否'}")
            if not redis_stats['connected'] and 'memory_cache_size' in redis_stats:
                logger.info(f"   内存缓存大小: {redis_stats['memory_cache_size']}")

    except Exception as e:
        logger.info(f"❌ 测试失败: {e}")
        import traceback
        traceback.print_exc()
    finally:
        vectorizer.cleanup()

if __name__ == '__main__':
    import time
    test_enhanced_vectorizer()