...
class HybridRetrievalSystem:
    ...
    def search(self, query: str, top_k: int = 20) -> list[RetrievalResult]:
        ...
        # 融合排序
        final_results = self._merge_and_rerank(bm25_results, vector_results, kg_results, top_k)
        ...
        for patent_id, word_freq in self.doc_freqs.items():
            ...
            for word in query_words:
                if word in word_freq and word in self.idf:
                    ...
                    avg_doc_len = np.mean([sum(freq.values()) for freq in self.doc_freqs.values()])
    ...
    def _merge_and_rerank(
        ...
        for result in bm25_results:
            if result.patent_id not in all_results:
                all_results[result.patent_id] = {"patent": result, "total_score": 0, "sources": []}
        ...
        sorted_results = sorted(
            all_results.items(), key=lambda x: x[1]["total_score"], reverse=True
        )
...
def test_hybrid_retrieval():
    ...
    # 测试查询
    test_queries = ["深度学习图像识别", "智能语音交互系统", "区块链数据存储", "人工智能专利"]
