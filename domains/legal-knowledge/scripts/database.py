...
class Category(BaseModel):
    ...
    is_sub_folder = peewee.BooleanField(default=False)
...
def update_status() -> None:
    ...
    req.request.search_type = '1,9'
    for param in params:
        req.request.params = [param]
        for item in req.law_list():
            ...
            if not laws:
                logger.info(f"{title} 不存在")
                continue
            if len(laws) != 1:
                logger.info(f"{title} 存在两份数据，请手动处理")
                continue
            law = laws[0]
...
def update_status_same_name() -> None:
