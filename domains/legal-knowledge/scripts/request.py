...
class RequestManager:
    ...
    def __init__(self) -> None:
        ...
        self.search_type = '1,9'

    def get_law_list(self, page=1):
        params = self.params + [
            ('search_type', f'title;accurate;{self.search_type}'),
            ('sort_tr', 'f_bbrq_s;desc'),
            ('gbrq_start', ''),
            ('gbrq_end', ''),
            ('sxrq_start', ''),
            ('sxrq_end', ''),
    ...
    def parse(self, result, detail) -> tuple[str, str, list[str]:
        ...
        for n, line in enumerate(lines):
            ...
            if is_desc and re.search(r"目.*录", line):
                is_desc = False
            if is_desc and is_start_line(line):
                is_desc = False

            if not has_desc and re.search('^法释', line):
                has_desc = True
...
class LawParser:
    ...
    def law_list(self):
        for i in range(1, 60):
            ret = self.request.get_law_list(i)
            arr = ret['result']['data']
            if len(arr) == 0:
                break
            yield from arr
