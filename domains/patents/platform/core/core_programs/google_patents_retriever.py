...
class GooglePatentsRetriever:
    ...
    async def analyze_patents(self, patents: list[PatentData]) -> dict[str, Any]:
        ...
        analysis['statistics'] = {
            ...
            'unique_classifications': len({p.classification for p in patents if p.classification})
        }
    ...
    # 缓存相关方法
    def _generate_cache_key(self, query: str, max_results: int, filters: dict[str, Any] | None, priority: PatentPriority) -> str:
        """生成缓存键"""
        cache_data = {
            'query': query,
