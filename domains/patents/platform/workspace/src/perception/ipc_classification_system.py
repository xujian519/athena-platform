...
class IPCClassificationSystem:
    ...
    def load_ipc_data(self):
        """加载IPC分类数据"""
        try:
            with open(self.ipc_data_path, 'r', encoding='utf-8') as f:
    ...
    def _generate_match_summary(self, matched_codes: List[IPCClassification], scores: List[float]) -> str:
        ...
        # 多领域分析
        sections = set(ipc.section for ipc in matched_codes)
        if len(sections) > 1:
    ...
    def _analyze_novelty_implications(self, matched_codes: List[IPCClassification]) -> Dict[str, Any]:
        ...
        # 新颖性考虑
        novelty_guidelines = self.ipc_data.get('novelty_analysis_guidelines', {})
...
def test_ipc_system():
    ...
    logger.info(f"匹配的IPC分类:")
    for i, (ipc, score) in enumerate(zip(match_result.matched_codes, match_result.confidence_scores)):
        logger.info(f"  {i+1}. {ipc.code} {ipc.name} (置信度: {score:.2f})")

    # 测试IPC比较
    logger.info(f"\n🔄 IPC分类比较测试:")
    comparison = ipc_system.compare_ipc_classes('G06F', 'A61')
    logger.info(f"G06F vs A61:")
