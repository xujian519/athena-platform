...
from cognitive_decision_manager import CognitiveDecisionManager
...
class AttentionFusion(nn.Module):
    ...
    def forward(self, features: list[torch.Tensor]) -> torch.Tensor:
        # 投影到统一维度
        projected_features = []
        for i, (feature, projection) in enumerate(zip(features, self.projections, strict=False)):
...
class HybridReasoningEngine:
    ...
    async def hybrid_reason(self,
                           ...
                           context: dict[str, Any] | None = None) -> HybridReasoningResult:
        """执行混合推理"""
