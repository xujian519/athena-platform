...
class ClaimGenerator:
    ...
    async def generate_independent_claim(
        ...
        options: ClaimGenerationOptions | None = None
    ) -> str:
        """
        生成独立权利要求
...
async def test_claim_generator():
    """测试权利要求生成器"""
    from core.patents.data_structures import InventionType, InventionUnderstanding, TechnicalFeature
    ...
    print(f"\n✅ 生成 {len(claims)} 条权利要求:\n")
    for _i, claim in enumerate(claims, 1):
        print(f"{claim}\n")
