...
try:
    # 相对导入（作为模块使用时）
    from .claim_generator import ClaimGenerationOptions, ClaimGenerator
    from .description_writer import DescriptionWriter
    from .disclosure_parser import DisclosureDocument, DisclosureDocumentParser
    ...
    # 绝对导入（直接运行时）
    from core.patents.drafting.claim_generator import ClaimGenerationOptions, ClaimGenerator
    from core.patents.drafting.description_writer import DescriptionWriter
    from core.patents.drafting.disclosure_parser import DisclosureDocumentParser
...
class PatentDrafter:
    ...
    async def draft_patent_application(
        self,
        disclosure_file: str,
        options: DraftingOptions | None = None
    ) -> PatentApplication:
        """
        完整专利申请文件撰写流程
...
async def test_patent_drafter():
    ...
    from core.patents.drafting.patent_drafter import DraftingOptions, PatentDrafter
...
"""
    ...
    try:
        ...
        for _i, claim in enumerate(application.claims, 1):
            print(f"{claim}")
