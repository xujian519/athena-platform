...
class ReadyOnDemandSystem:
    ...
    def _init_agents(self) -> Dict[AgentType, AgentConfig]:
        ...
        return {
            ...
            AgentType.YUNXI: AgentConfig(
                name="云熙",
                port=8087,
                memory_mb=100,
                startup_time=4.0,
                idle_timeout=300,  # 5分钟
                capabilities=["IP管理", "案卷管理", "专利检索", "分类分析", "数据统计"]
            ),
    ...
    def _determine_agent(self, task_request: TaskRequest) -> AgentType:
        ...
        if task_request.preferred_agent:
            try:
                ...
                logger.debug(f"[ready_on_demand_system] ValueError: {e}")

        # 专利法律相关 -> 小娜
        if any(keyword in task_type_lower or keyword in content_lower
               for keyword in ["专利", "权利要求", "审查", "侵权", "法律", "撰写", "答复"]):
            return AgentType.XIAONA

        # IP管理相关 -> 云熙
        elif any(keyword in task_type_lower or keyword in content_lower
...
📞 如需详细法律咨询，请提供更多技术细节。"""
        ...
        elif target_agent == AgentType.YUNXI:
            return f"""云熙管理系统处理：

📂 IP管理任务：{task_request.content}
...
📝 内容已准备就绪，可以发布使用！"""
        ...
        while True:
            try:
                ...
                for agent_type, config in self.agents.items():
                    ...
                    # 检查空闲超时
                    if (agent_key in self.last_activity and
    ...
    def get_running_agents(self) -> List[str]:
        ...
        for agent_type, config in self.agents.items():
            ...
            # 检查其他智能体
            elif (agent_key in self.last_activity and
                  (config.idle_timeout == 0 or
                   (current_time - self.last_activity[agent_key]).total_seconds() < config.idle_timeout)):
                running.append(config.name)
...
class OnDemandAI:
    ...
    async def ip_management(self, query: str) -> str:
        ...
        task_request = TaskRequest(
            task_type="IP管理",
            content=query,
            preferred_agent="yunxi"
        )
        await self.system.submit_task(task_request)
        await asyncio.sleep(2)
        return await self.system._generate_response(task_request, AgentType.YUNXI)
