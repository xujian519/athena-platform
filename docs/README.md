...
```bash
# 完整启动小诺平台
./dev/scripts/xiaonuo_quick_start.sh

# 或者
./dev/scripts/xiaonuo_quick_start.sh 启动
```

### 方式二：使用Python启动脚本
```bash
# 启动小诺平台
python3 dev/scripts/xiaonuo_unified_startup.py 启动平台

# 检查系统状态
python3 dev/scripts/xiaonuo_system_checker.py
```
...
│   └── 📁 infrastructure/docker/                         # Docker初始化脚本
├── 📁 configs/                            # 配置文件
...
├── 📁 docs/                               # 文档
│   └── 📊 reports/                        # 报告文件
└── 📁 modules/storage/storage-system/                     # 存储系统
...
```bash
# 快速检查
./dev/scripts/xiaonuo_quick_start.sh 检查
