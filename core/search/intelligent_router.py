...
废弃日期: 2026-01-11
废弃原因: 系统已迁移到 PostgreSQL + Qdrant 架构
影响范围: 本文件包含NebulaGraph相关代码引用
建议操作: 新项目请使用 core/search/ 下的新检索组件

原功能说明:
智能检索路由系统
Intelligent Retrieval Router System
...
class KGRetriever:
    ...
    def __init__(self, nebula_host: Optional[str] = None, nebula_port: Optional[int] = None):
        """初始化知识图谱检索器

        Args:
            nebula_host: NebulaGraph主机地址,默认从环境变量读取
            nebula_port: NebulaGraph端口,默认从环境变量读取
        """
        import os

        if nebula_host is None:
            nebula_host = os.getenv("NEBULA_HOST", "localhost")
        if nebula_port is None:
            nebula_port = int(os.getenv("NEBULA_PORT", "19669"))

        self.config = Config()
        self.pool = ConnectionPool()

        try:
            self.pool.init([(nebula_host, nebula_port)], self.config)
            self.session = self.pool.get_session("root", "nebula")
            logger.info("✅ Nebula连接成功")
        except Exception as e:
            logger.error(f"❌ Nebula连接失败: {e}")
            self.session = None
    ...
    def query_patent_kg(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        try:
            # 使用patent_knowledge空间
            self.session.execute("USE patent_knowledge")

            # 简单查询示例
            result = self.session.execute(
                f"MATCH (v:patent) RETURN v.number, v.title, v.status LIMIT {limit}"
            )
            ...
            for row in result:
                results.append(
                    SearchResult(
                        ...
                        score=0.8,
                        metadata={"number": row[0], "status": row[2]},
                    )
                )
        except Exception as e:
    ...
    def query_legal_kg(self, query: str, limit: int = 10) -> list[SearchResult]:
        ...
        try:
            # 使用athena_graph或legal_knowledge空间
            self.session.execute("USE athena_graph")

            # 查询法律规则
            result = self.session.execute(f"MATCH (v) RETURN v LIMIT {limit}")

            for row in result:
                results.append(
                    SearchResult(
                        content=str(row[0]), source=DataSource.LEGAL_KG, score=0.8, metadata={}
                    )
                )
        except Exception as e:
