...
from core.framework.memory.cache_utils import (
    BatchProcessor,
    LRUCache,
)
from core.framework.memory.pattern_index_manager import PatternIndexManager
from core.framework.memory.retry_utils import RetryConfig, async_retry
from core.framework.memory.security_utils import (
    InputValidator,
    PathValidator,
    SecurityError,
    SensitiveDataFilter,
)
from core.framework.memory.serializers.markdown_serializer import PatternMarkdownSerializer
from core.framework.memory.type_utils import safe_domain_getter
from core.framework.memory.vector_workflow_retriever import VectorWorkflowRetriever
from core.framework.memory.workflow_extractor import WorkflowExtractor
from core.framework.memory.workflow_pattern import TaskTrajectory, WorkflowPattern
from core.framework.memory.workflow_retriever import RetrievalResult, WorkflowRetriever
...
class CrossTaskWorkflowMemory:
    ...
    def __init__(
        ...
        # 尝试初始化向量检索器(如果启用)
        self.vector_retriever: Optional[VectorWorkflowRetriever] = None
    ...
    async def _generate_task_embedding(self, task: Any) -> np.Optional[ndarray]:
        """
        生成任务的向量表示
