...
async def _analyze_claim_relevance(claims: str, papers: Optional[list[dict])] -> dict[str, Any]:
    """
    分析论文与权利要求的相关性
...
def _calculate_trend(yearly_data: Optional[dict[int, int)]] -> str:
    """计算趋势"""
...
def _calculate_growth_rate(yearly_data: Optional[dict[int, int)]] -> str:
    """计算增长率"""
    values = list(yearly_data.values())
    if len(values) < 2:
        return 0.0
