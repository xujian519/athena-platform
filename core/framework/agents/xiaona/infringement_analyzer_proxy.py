...
"""
    ...
    async def compare_features(
        ...
        product_description: Optional[Dict[str, Any]) -> Dict[str, Any]:
        """
        将产品/方法与权利要求比对（LLM版本）
    ...
    def _build_feature_comparison_prompt(
        ...
        product_description: Optional[Dict[str, Any]) -> str:
        """构建特征比对提示词"""
        product_name = product_description.get("product_name", "未知产品")
        product_features = product_description.get("features", {})
...
"""
    ...
    async def determine_infringement(
        self,
        comparisons: Optional[[List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    def _build_infringement_determination_prompt(
        self,
        comparisons: Optional[[List[Dict[str, Any]]) -> str:
        """构建侵权判定提示词"""
        return f"""# 任务：专利侵权判定
...
"""
    ...
    def _feature_covered(
        self,
        feature: str,
        product_features: Optional[Dict[str, Any]) -> str:
        """判断特征是否被覆盖"""
        # 简化版判断
    ...
    def _find_equivalent_features(
        ...
        product_features: Optional[Dict[str, Any]) -> List[str]:
        """查找等同特征"""
        # 简化版等同特征判定
        equivalent = []
    ...
    def _get_legal_basis(
        ...
        equivalent_claims: Optional[[List[int]]) -> str:
        """获取法律依据"""
