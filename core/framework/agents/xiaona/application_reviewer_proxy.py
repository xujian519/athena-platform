...
"""
    ...
    def _build_format_review_prompt(self, application: Optional[Dict[str, Any)] -> str:
        """构建格式审查提示词"""
...
"""
    ...
    def _parse_format_review_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "provided_documents": Optional[[],]

                "missing_documents": Optional[[],]
    ...
    def _review_disclosure_by_rules(
        ...
        application.get("invention_summary", "")
    ...
    def _build_disclosure_review_prompt(self, application: Optional[Dict[str, Any)] -> str:
        """构建披露审查提示词"""
...
"""
    ...
    def _build_claims_review_prompt(self, application: Optional[Dict[str, Any)] -> str:
        """构建权利要求审查提示词"""
        claims_text = application.get("claims", "")
...
"""
    ...
    def _build_specification_review_prompt(self, application: Optional[Dict[str, Any)] -> str:
        """构建说明书审查提示词"""
        specification = application.get("specification", "")
        embodiments = application.get("embodiments", [)
        drawings = application.get("drawings", [)
...
"""
    ...
    def _assess_claims_clarity(
        self,
        claims_list: Optional[[List[Dict[str, Any]]) -> Dict[str, Any]:
        """评估权利要求清晰度"""
    ...
    def _assess_enablement(
        self,
        specification_text: str,
        embodiments: Optional[[List[Any]]) -> Dict[str, Any]:
        """评估充分公开（本领域技术人员能否实现）"""
        if not embodiments:
            return {
                "score": 0.3,
    ...
    def _assess_drawings_support(
        self,
        specification_text: str,
        drawings: Optional[[List[Any]]) -> Dict[str, Any]:
        """评估附图支持"""
        if not drawings:
            return {
                "score": 0.7,
    ...
    def _generate_specification_suggestions(
        self,
        specification_review: Optional[Dict[str, Any]) -> List[str]:
        """生成说明书改进建议"""
        suggestions = []
