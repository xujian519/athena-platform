...
{{
    "invalidation_grounds_analysis": {{
        ...
        ,
        "total_grounds": 3,
        "ground_strengths": Optional[[],]

        "recommended_grounds": Optional[List]

    }},
    "evidence_strategy": {{
        "evidence_categories": Optional[[],]

        "collection_plan": Optional[[],]
...
"""
    ...
    async def analyze_invalidation_grounds(
        ...
        references: Optional[List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        分析无效理由（LLM版本）
    ...
    def _build_grounds_analysis_prompt(
        ...
        references: Optional[List[Dict[str, Any]]) -> str:
        """构建无效理由分析提示词"""
...
"""
    ...
    def _parse_grounds_analysis_response(self, response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "valid_grounds": Optional[[],]

                "total_grounds": 0,
                "ground_strengths": Optional[[],]

                "recommended_grounds": Optional[[],]
    ...
    async def develop_evidence_strategy(
        ...
        existing_references: Optional[List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        制定证据搜集策略（LLM版本）
    ...
    async def _develop_evidence_strategy_by_rules(
        ...
        _existing_references: Optional[List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        基于规则的证据搜集策略（降级方案）
    ...
    def _build_evidence_strategy_prompt(
        ...
        existing_references: Optional[List[Dict[str, Any]]) -> str:
        """构建证据策略提示词"""
