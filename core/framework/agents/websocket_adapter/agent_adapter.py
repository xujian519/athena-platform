...
from .client import AgentType, MessageType, WebSocketClient
...
class BaseAgentAdapter(ABC):
    ...
    def _register_handlers(self) -> str:
        """注册消息处理器"""
    ...
    async def _handle_task(self, message) -> str:
        """
        处理任务消息
    ...
    async def _handle_query(self, message) -> str:
        """
        处理查询消息
    ...
    async def _handle_ping(self, message) -> str:
        """处理心跳消息"""
        # Pong消息会由客户端自动处理
        pass
    ...
    async def _progress_callback(self, progress: int, status: str, current_step: str = "", total_steps: int = 0) -> str:
        """
        进度回调函数
    ...
    async def notify(self, level: str, title: str, body: str) -> str:
        """
        发送通知消息
