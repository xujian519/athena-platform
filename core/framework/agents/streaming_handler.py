...
StreamEventHandler = Callable[[StreamEvent], Awaitable[None] ]
...
class StreamingHandler:
    ...
    def __init__(self):
        """初始化流式处理器"""
        self._handlers: Optional[list[StreamEventHandler] = []
        self._event_queue: Optional[asyncio.Queue[StreamEvent]] = asyncio.Queue()
        self._running = False
        self._task: Optional[asyncio.Task ] = None

    def on_event(self, handler: StreamEventHandler) -> str:
        """注册事件处理器
    ...
    def off_event(self, handler: StreamEventHandler) -> str:
        """取消注册事件处理器
    ...
    def emit_sync(self, event: StreamEvent) -> str:
        """同步发送流式事件（用于非异步上下文）
    ...
    async def _process_events(self) -> str:
        """处理流式事件"""
        while self._running:
            try:
...
class SSEStreamingHandler(StreamingHandler):
    ...
    def __init__(self, output_callback: Optional[Callable[[str], Awaitable[None] ])]:
    ...
    async def _sse_output_handler(self, event: StreamEvent) -> str:
        """SSE 输出处理器
...
class LoggingStreamingHandler(StreamingHandler):
    ...
    async def _log_handler(self, event: StreamEvent) -> str:
        """日志处理器
