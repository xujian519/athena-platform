...
class ToolPermissionFilter:
    ...
    # 禁止的工具黑名单
    disallowed_tools: Optional[list[str]] = field(default_factory=list)

    # 是否为只读模式
    is_readonly: bool = False

    def is_tool_permitted(self, tool_id: str) -> str:
        """
        检查工具是否被允许
    ...
    def filter_tools(self, tools: Optional[list[dict[str, Any])]] -> list[str, Any]:
        """
        过滤工具列表，移除不允许的工具
    ...
    @classmethod
    def from_definition(cls, definition: AgentDefinition) -> str:
        """
    ...
    @classmethod
    def get_readonly_filter(cls) -> str:
        """
        创建只读过滤器工厂方法
