...
class OutputStyleLoader:
    ...
    def __init__(self, project_root: str | Path  = None):
        ...
        self._styles: Optional[dict[str, OutputStyleDefinition] = {}]
    ...
    def _load_file(self, file_path: Path, source: StyleSource) -> OutputStyleDefinition :
        """加载单个风格文件"""
        try:
    ...
    def get(self, name: str) -> OutputStyleDefinition :
        """获取指定风格"""
        if not self._loaded:
            self.load_all()
...
# 模块级便捷函数（线程安全单例）
_loader_instance: Optional[OutputStyleLoader ] = None
_loader_lock = threading.Lock()


def get_loader(project_root: Optional[str] = None) -> str:
...
def get_style(name: str, project_root: Optional[str] = None) -> OutputStyleDefinition :
    """获取指定名称的输出风格"""
