...
class Task:
    ...
    result: Optional[TaskResult] = None  # 执行结果
    agent_preference: Optional[AgentType] = None  # 优先使用的智能体
    metadata: dict[str, Any] = field(default_factory=dict)  # 元数据
...
class AgentRouter:
    ...
    def route(self, capability: Capability, preference: Optional[AgentType] = None) -> AgentType:
        """
        路由到合适的智能体
