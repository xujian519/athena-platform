...
class Task:
    ...
    estimated_duration: Optional[timedelta] = None  # 预估持续时间
...
class ResourceManager:
    ...
    def request_resource(
        ...
        agent_id: str,
        resource_type: str,
        requirements: dict[str, Any],        duration: Optional[timedelta] = None,
    ) -> Optional[str]:
        """请求资源"""
        with self._lock:
            # 查找合适资源
...
class MultiAgentCollaborationFramework:
    ...
    def create_task(self, task_definition: dict[str, Any]) -> Optional[Task]:
        """创建协作任务"""
        try:
            task = Task(
