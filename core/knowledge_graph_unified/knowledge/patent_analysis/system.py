...
class PatentProject:
    ...
    evaluation_result: Optional[PatentEvaluationResult] = None
...
class PatentAnalysisSystem:
    ...
    _instance: Optional[PatentAnalysisSystem] = None

    def __init__(self, config: Optional[SystemConfiguration] = None):
        self.config = config or SystemConfiguration()
        self.projects: dict[str, PatentProject] = {}
        self.analyzer: Optional[PatentAnalyzer] = None
        self.evaluator: Optional[PatentEvaluator] = None
        self.rewriter: Optional[PatentRewriter] = None
        self._initialized = False

    @classmethod
    async def initialize(cls, config: Optional[SystemConfiguration] = None):
        """初始化系统"""
    ...
    async def create_project(
        ...
        title: str,
        description: str,
        patent_data: dict[str, Any],        analysis_types: Optional[list[AnalysisType]] = None,
        rewrite_targets: Optional[list[RewriteTarget]] = None,
    ) -> str:
        """
        创建专利项目
    ...
    async def rewrite_project(
        self, project_id: str, rewrite_mode: Optional[RewriteMode] = None
    ) -> list[RewriteResult]:
        """
        重写项目
    ...
    async def get_project(self, project_id: str) -> Optional[PatentProject]:
        """获取项目"""
        return self.projects.get(project_id)

    async def list_projects(self, status: Optional[ProjectStatus] = None) -> list[PatentProject]:
        """列出项目"""
