...
class OrchestrationModule:
    ...
    async def execute_parallel(
        ...
        async def run_one(idx: int, task: Callable, name: str) -> TaskResult:
            return await self._execute_task(name, task(), idx + 1)

        coros = [run_one(i, task, name) for i, (task, name) in enumerate(zip(tasks, task_names))]
    ...
    async def _draft_response_internal(
        ...
        writer: Any,
        office_action: dict[str, Any],
        oa_analysis: dict[str, Any],
        citation_analysis: dict[str, Any],
        prior_art: dict[str, Any] | None,
    ) -> dict[str, Any]:
        """内部方法：撰写答复意见"""
        try:
            context = {
