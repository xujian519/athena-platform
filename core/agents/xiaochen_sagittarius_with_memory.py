...
"""
小宸·星河射手 - 集成记忆系统
Xiaochen Sagittarius with Integrated Memory
...
class XiaochenSagittariusAgent(MemoryEnabledAgent):
    """小宸·星河射手 - 集成记忆系统"""

    def __init__(self):
        super().__init__(
            agent_id="xiaochen_sagittarius", agent_type=AgentType.XIAOCHEN, role=AgentRole.ASSISTANT
        )

        # 小宸特有属性
        self.media_domains = ["内容策划", "平台运营", "传播策略", "用户增长", "数据分析"]
        self.motto = "如星河射手般精准,内容直击目标用户"
        self.content_portfolio = []
    ...
    async def initialize(self, memory_system: UnifiedAgentMemorySystem):
        ...
        # 加载自媒体运营知识
        await self._load_media_operation_knowledge()

        logger.info("🎯 小宸·星河射手已初始化,拥有专业的自媒体运营能力")

    async def _load_media_operation_knowledge(self):
        """加载自媒体运营专业知识"""
        media_memories = [
            {
                "content": "我是小宸·星河射手,自媒体运营专家,专注多平台内容传播策略",
                "type": "identity",
                "importance": 1.0,
            },
            {
                "content": "我的信条:如星河射手般精准,内容直击目标用户",
                "type": "motto",
                "importance": 1.0,
            },
            {
                "content": "专业领域:内容策划、平台运营、传播策略、用户增长、数据分析",
                "type": "expertise",
                "importance": 0.95,
            },
            {
                "content": "运营理念:内容为王、用户为本、数据驱动、持续创新",
                "type": "operation_philosophy",
                "importance": 0.9,
            },
            {
                "content": "每个平台都有独特生态,需要差异化运营策略",
                "type": "platform_strategy",
                "importance": 0.9,
            },
            {
                "content": "优质内容是自媒体运营的核心,需要持续创新和优化",
                "type": "content_wisdom",
                "importance": 0.85,
            },
            {
                "content": "数据是运营的眼睛,通过数据分析洞察用户需求",
                "type": "data_insight",
                "importance": 0.9,
            },
        ]
        ...
        for memory in media_memories:
            await self.memory_system.store_memory(
                ...
                work_related=True,
                tags=["自媒体", "运营", "内容", "传播"],
                metadata={"category": "media_operation", "domain": "social_media"},
    ...
    async def generate_response(self, user_input: str, **kwargs) -> str:
        ...
        if media_need == "content_strategy":
            return await self._handle_content_strategy(user_input)
        elif media_need == "platform_operation":
            return await self._handle_platform_operation(user_input)
        elif media_need == "user_growth":
            return await self._handle_user_growth(user_input)
        elif media_need == "data_analysis":
            return await self._handle_data_analysis(user_input)
        elif media_need == "promotion":
            return await self._handle_promotion(user_input)
        else:
            return await self._general_media_response(user_input)

    async def _analyze_media_need(self, user_input: str) -> str:
        """分析用户的自媒体运营需求"""
    ...
    async def _handle_content_strategy(self, request: str) -> str:
        """处理内容策略"""
        # 获取相关记忆
        await self.recall_memories("内容", limit=3, memory_type=MemoryType.PROFESSIONAL)
        ...
        # 记录策划请求
        await self.memory_system.store_memory(
            f"内容策略策划请求:{request[:100]}...",
            MemoryType.PROFESSIONAL,
            importance=0.8,
            work_related=True,
            tags=["内容策划", "自媒体"],
            metadata={"service_type": "content_strategy"},
        )
    ...
    async def _general_media_response(self, user_input: str) -> str:
        """生成一般性自媒体运营回应"""
        responses = [
            "自媒体运营需要策略和坚持。小宸会帮您制定最适合的运营方案,让内容直击目标用户。",
            "每个成功的自媒体账号背后都有科学的运营方法。让我们一起打造您的影响力吧!",
            "内容创作是基础,运营策略是关键。小宸会为您提供全方位的运营支持。",
            "如星河射手般精准,这是小宸对自媒体运营的承诺。让我们一起创造爆款内容吧!",
        ]
    ...
    async def get_media_operation_overview(self) -> dict[str, Any]:
        """获取自媒体运营概览"""
        ...
        overview = {
            ...
            "motto": self.motto,
            "content_count": len(self.content_portfolio),
            "strategy_count": len(self.platform_strategies),
            "campaign_count": len(self.campaign_history),
            "memory_stats": stats,
            "core_capabilities": [
                "内容策划创作",
                "多平台运营",
...
# 测试函数
async def test_xiaochen_with_memory():
    """测试小宸带记忆功能"""
    print("🎯 测试小宸·星河射手 - 集成记忆系统...")

    from ..memory.unified_agent_memory_system import UnifiedAgentMemorySystem

    # 创建小宸
    xiaochen = XiaochenSagittariusAgent()
    ...
    try:
        ...
        # 初始化小宸
        await xiaochen.initialize(memory_system)
        print("✅ 小宸初始化成功")
        ...
        for inquiry in inquiries:
            print(f"\n📝 用户: {inquiry}")
            response = await xiaochen.process_input(inquiry)
            print(f"🎯 小宸: {response}")

        # 显示运营概览
        print("\n📊 自媒体运营概览:")
        overview = await xiaochen.get_media_operation_overview()

        print(f"  运营领域: {len(overview.get('media_domains', []))}个")
        print(f"  座右铭: {overview.get('motto', '')}")
        ...
        await xiaochen.shutdown()


if __name__ == "__main__":
    asyncio.run(test_xiaochen_with_memory())
