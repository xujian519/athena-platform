...
class GatewayClient:
    ...
    def __init__(self, config: GatewayClientConfig):
        self.config = config
        self._websocket: ws_client.WebSocketClientProtocol | None = None
    ...
    async def _send_message_raw(self, data: str) -> None:
        """发送原始消息"""
        if self._websocket:
...
async def get_gateway_client(config: GatewayClientConfig | None = None) -> GatewayClient:
    """
    获取Gateway客户端单例
