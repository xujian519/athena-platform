...
class StructuredDecisionLogger:
    ...
    def log_decision_start(self, task_id: str, task_description: str, context: Optional[dict] = None) -> Any:
        """记录决策开始"""
    ...
    def log_conflict_detection(
        self, task_id: str, has_conflict: bool, conflict_type: str, conflict_details: Optional[dict] = None
    ):
        """记录冲突检测"""
        entry = DecisionLogEntry(
            timestamp=datetime.now().isoformat(),
...
# 便捷函数
def log_decision_process(task_id: str, decision_data: dict[str, Any]) -> Optional[Path]:
    """
    便捷函数:记录完整决策过程
