...
class FileCache:
    ...
    async def _load_entry_from_disk(self, cache_key: str) -> Optional[FileCacheEntry]:
        """从磁盘加载缓存条目"""
...
async def cached_file_operation(
    file_path: Optional[str] = None, operation: Optional[callable] = None, ttl: Optional[int] = None
) -> bytes:
    """
    执行缓存文件操作
