...
try:
    ...
    # 尝试获取向量存储提供者
    vector_provider = DIContainer.resolve(VectorStoreProvider)
...
from services.agent_services.vector_db.optimized_qdrant_client import OptimizedQdrantClient
        vector_provider = OptimizedQdrantClient()
except ImportError:
...
class HybridStorageManager:
    ...
    def search_vectors(
        self, query_vector: Optional[list[float], limit: int = 10, filters: dict[str, Any] = None
    ) -> dict[str, Any]:
        """跨存储系统搜索向量"""
