...
from .progressive_loader import (  # DEPRECATED: will be removed in v3.0, use core.api.prompt_system_routes.generate_prompt()
    ComplexityLevel,
...
def get_minimal_prompt() -> str:
    ...
    """
    loader = get_loader()
    return loader.get_minimal_context()  # type: ignore[no-any-return]
...
def get_capability_prompt(capabilities: list[str]) -> str:
    ...
    """
    loader = get_loader()
    return loader.get_capability_context(capabilities)  # type: ignore[no-any-return]
...
def get_business_prompt(task_type: str) -> str:
    ...
    """
    loader = get_loader()
    return loader.get_business_context(TaskType(task_type))  # type: ignore[no-any-return]
...
def get_system_stats() -> dict:
    """获取系统统计信息"""
    loader = get_loader()
    return loader.get_stats()  # type: ignore[no-any-return]
