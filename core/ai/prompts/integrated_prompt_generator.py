...
warnings.warn(
    "integrated_prompt_generator is deprecated and will be removed in v3.0. "
...
def _sync_call_async(async_func, *args, **kwargs):
    ...
    用于修复 deprecated 模块中的 async/sync 错配，
    避免 coroutine 对象被当作字符串使用。
    """
...
class IntegratedPromptGenerator:
    ...
    def generate(
        ...
        if scenario_rule.lyra_optimization and self.unified_prompt_manager:
            try:
                # 优化system prompt
                system_prompt = _sync_call_async(
                    self.unified_prompt_manager.optimize_prompt,
                    system_prompt,
                    f"scenario_{scenario_rule.domain}_{scenario_rule.task_type}",
                )
                lyra_optimized = True
    ...
    def _get_agent_role_prompt(self, agent_level: str) -> Optional[str]:
        ...
        try:
            ...
            # 导入PromptFormat
            from core.ai.prompts.unified_prompt_manager import PromptFormat

            # 使用UnifiedPromptManager加载提示词
            role_prompt = _sync_call_async(
                self.unified_prompt_manager.load_prompt, agent_name, PromptFormat.MARKDOWN
            )
    ...
    def generate_with_legal_context(
        ...
        scenario_rule: ScenarioRule,
        user_input: str,
        legal_basis: Optional[list[dict[str, Any],        reference_cases: list[dict[str, Any],        additional_variables: dict[str, Any] = None,
    ) -> IntegratedPrompt:
        """
        生成包含法律上下文的集成提示词
