...
class StreamProcessorFactory:
    ...
    @classmethod
    def create_processor(cls, stream_type: StreamType, config: Optional[StreamConfig] = None) -> Any:
        """
        创建流处理器实例
...
class PerceptionBuilder:
    ...
    def with_processor(
        self, input_type: InputType, processor_config: Optional[dict] = None
    ) -> PerceptionBuilder:
        """
        添加处理器
...
def create_text_processor(config: Optional[dict] = None) -> Any:
    """创建文本处理器(便捷函数)"""
    return ProcessorFactory.create_processor(InputType.TEXT, config)


def create_image_processor(config: Optional[dict] = None) -> Any:
    """创建图像处理器(便捷函数)"""
    return ProcessorFactory.create_processor(InputType.IMAGE, config)


def create_audio_processor(config: Optional[dict] = None) -> Any:
    """创建音频处理器(便捷函数)"""
    return ProcessorFactory.create_processor(InputType.AUDIO, config)


def create_video_processor(config: Optional[dict] = None) -> Any:
    """创建视频处理器(便捷函数)"""
    return ProcessorFactory.create_processor(InputType.VIDEO, config)


def create_multimodal_processor(config: Optional[dict] = None) -> Any:
    """创建多模态处理器(便捷函数)"""
