...
class MetricsCollector:
    ...
    def record_histogram(self, name: str, value: float, labels: Optional[dict[str, str]] = None) -> Any:
        """记录直方图"""
        with self.lock:
            key = self._make_key(name, labels)
    ...
    def record_timer(self, name: str, duration: float, labels: Optional[dict[str, str]] = None) -> Any:
        """记录计时器"""
        with self.lock:
            key = self._make_key(name, labels)
...
# 全局监控器实例
_global_monitor: Optional[PerceptionMonitor] = None
