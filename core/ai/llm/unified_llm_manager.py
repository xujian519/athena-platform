...
from core.ai.llm.cache_warmer import get_cache_warmer
from core.ai.llm.cost_monitor import get_cost_monitor
from core.ai.llm.model_registry import ModelCapabilityRegistry, get_model_registry
from core.ai.llm.model_selector import IntelligentModelSelector
from core.ai.llm.prometheus_metrics import get_metrics
from core.ai.llm.response_cache import get_response_cache

# 智能模型路由 (Hermes Agent 设计模式)
try:
    from core.ai.llm.smart_model_routing import RoutingDecision, SmartModelRouter
...
class UnifiedLLMManager:
    ...
    def __init__(self, registry: ModelCapabilityRegistry = None):
        ...
        self.smart_router: Optional[SmartModelRouter] = None
    ...
    async def _load_glm_adapters(self):
        ...
        for model_id in glm_models:
            try:
                # 动态导入以避免循环依赖
                from core.ai.llm.adapters.glm_adapter import GLMAdapter
    ...
    async def _load_deepseek_adapters(self):
        ...
        for model_id in deepseek_models:
            try:
                from core.ai.llm.adapters.deepseek_adapter import DeepSeekAdapter
    ...
    async def _load_local_adapters(self):
        ...
        for model_id in local_models:
            try:
                from core.ai.llm.adapters.local_llm_adapter import LocalLLMAdapter
    ...
    async def _load_ollama_adapters(self):
        ...
        for model_id in ollama_models:
            try:
                from core.ai.llm.adapters.ollama_adapter import OllamaAdapter
    ...
    async def _load_qwen_adapters(self):
        ...
        for model_id in qwen_models:
            try:
                from core.ai.llm.adapters.qwen_adapter import QwenAdapter
    ...
    async def generate(
        self, message: str, task_type: str, context: Optional[dict] = None, **kwargs
    ) -> LLMResponse:
        """
        统一生成接口
