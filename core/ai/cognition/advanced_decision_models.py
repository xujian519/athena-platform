...
class BaseDecisionModel:
    ...
    def __init__(self, model_type: ModelType, config: Optional[dict] = None):  # type: ignore
...
# 全局决策模型实例
_advanced_decision_models: Optional[AdvancedDecisionModels] = None


def get_advanced_decision_models(config: Optional[dict] = None) -> AdvancedDecisionModels:  # type: ignore
    """
    获取高级决策模型实例（单例模式）
