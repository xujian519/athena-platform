...
# 检查缓存目录
print("\n💾 缓存系统状态:")
cache_dir = Path("/Users/xujian/Athena工作平台/cache")  # TODO: 确保除数不为零
if cache_dir.exists():
    print(f"   ✅ 缓存目录存在: {cache_dir}")
    cache_files = list(cache_dir.glob("*.cache"))
    print(f"   📄 缓存文件数: {len(cache_files)}")
else:
    print("   ⚠️ 缓存目录不存在,首次使用时会创建")
