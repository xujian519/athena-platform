...
class HybridEmbedding:
    """混合嵌入结果"""
    general: np.Optional[ndarray] = None  # BGE-M3嵌入 (1024维)
    patent: np.Optional[ndarray] = None  # PatentSBERTa嵌入 (768维)
    fused: np.Optional[ndarray] = None  # 融合嵌入 (可选)
...
class HybridPatentEmbeddingService:
    ...
    async def initialize(self) -> None:
        ...
        if self.enable_patent_sberta and self._patent_encoder is None:
            try:
                from core.ai.embedding.patent_sberta_encoder import (
                    get_patent_encoder,
                )
...
_global_hybrid_service: Optional[HybridPatentEmbeddingService] = None
