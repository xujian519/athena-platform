...
class AdvancedParameterExtractor:
    ...
    def _try_regex_extraction(self, query: str, schema: ParameterSchema) -> Optional[Any]:
        """尝试正则表达式提取"""
        # 基于参数类型定义正则模式
        patterns = {
    ...
    def _try_keyword_extraction(self, query: str, schema: ParameterSchema) -> Optional[Any]:
        """尝试关键词提取"""
        # 在查询中查找参数名附近的值
    ...
    def _try_range_extraction(self, query: str, schema: ParameterSchema) -> Optional[Any]:
        """尝试范围提取(用于枚举类型)"""
    ...
    def _infer_from_history(
        self, schema: ParameterSchema, history: list[dict[str, Any]) -> Optional[Any]]:
        """从历史对话中推理参数"""
        # 搜索最近的值
