...
from patent_knowledge_connector import PatentKnowledgeConnector

# 导入专家系统
from xiaona_patent_expert_system import PatentContext, PatentExpertSystem
...
class XiaonaPatentAnalyzer:
    ...
    def __init__(self):
        ...
        self.result_filter_threshold = 0.6
    ...
    async def initialize(self):
        ...
        await self.load_patent_knowledge_base()
        await self.setup_external_search_engines()
    ...
    async def _build_initial_search_query(self, request: PatentAnalysisRequest) -> str:
        """构建初始检索式"""
        # 提取技术特征
        tech_features = await self._extract_technical_features(request.invention_description)
        ...
        # 核心技术特征
        if keywords:
            search_components.append(" AND ".join([f'"{kw}"' for kw in keywords[:5]]))
    ...
    async def _optimize_search_query(
        ...
        """优化检索查询"""
        # 分析当前检索结果
        result_analysis = await self._analyze_search_results(iteration_result.filtered_results)

        # 基于分析优化查询
        optimizations = [
            self._add_technology_specific_terms,
            self._add_similar_patent_terms,
            self._add_inventive_step_clauses,
            self._exclude_common_concepts,
        ]

        optimized_query = current_query
        for optimization in optimizations:
            optimized_query = await optimization(optimized_query, result_analysis, request)
    ...
    async def _execute_search_iteration(
        ...
        # 执行专利数据库检索
        db_results = await self._search_patent_database(query)

        # 执行外部搜索引擎检索
        external_results = await self._search_external_engines(query)

        # 合并结果
        all_results = db_results + external_results

        # 语义相似度过滤
        filtered_results = await self._filter_by_similarity(
        ...
        for result in filtered_results:
            confidence = await self._calculate_patent_relevance(
