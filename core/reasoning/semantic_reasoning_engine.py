...
class SemanticReasoningEngine:
    ...
    def _init_embedding_model(self) -> Any:
        ...
        try:
            ...
            if torch.backends.mps.is_available():
                device = "mps"
                logger.info("🔥 检测到MPS支持,启用MPS加速")
            elif torch.cuda.is_available():
                device = "cuda"
                logger.info("🔥 检测到CUDA支持,启用CUDA加速")
            ...
            if model_path:
                ...
                logger.info("📥 本地模型不存在,使用HuggingFace远程模型")
                self.embedding_model = SentenceTransformer("BAAI/BAAI/bge-m3", device=device)
    ...
    async def reason(
        ...
        if ReasoningType.RULE_BASED in reasoning_types:
            rule_results = await self._rule_based_reasoning(query, context, domain)
            results.extend(rule_results)

        # 语义推理
        if ReasoningType.SEMANTIC in reasoning_types and self.embedding_model:
            semantic_results = await self._semantic_reasoning(query, context, domain)
            results.extend(semantic_results)

        # 基于案例的推理
        if ReasoningType.CASE_BASED in reasoning_types:
            case_results = await self._case_based_reasoning(query, context, domain)
            results.extend(case_results)

        # 因果推理
        if ReasoningType.CAUSAL in reasoning_types:
            causal_results = await self._causal_reasoning(query, context, domain)
            results.extend(causal_results)

        # 混合推理
        if ReasoningType.HYBRID in reasoning_types and len(results) > 1:
            hybrid_results = await self._hybrid_reasoning(results, query, context)
    ...
    async def _rule_based_reasoning(
        self, query: str, context: str, domain: str,
    ) -> list[ReasoningResult]:
        """基于规则的推理"""
        results = []
        ...
        for rule_id, rule in self.reasoning_rules.items():
            ...
            if satisfaction_rate >= 0.6:  # 至少满足60%的条件
                # 计算置信度
                confidence = rule.confidence_base * satisfaction_rate
    ...
    async def _semantic_reasoning(
        ...
        try:
            ...
            if domain and domain in self.ontology_concepts:
                ontology_results = await self._ontology_reasoning(query, query_vector, domain)
        ...
        for case_id, case in self.case_database.items():
            ...
            if similarity > 0.5:  # 相似度阈值
                result = ReasoningResult(
                    ...
                    confidence=similarity * case.get("precedent_value", 0.8),
                    evidence=[f"案例ID: {case_id}", f"相似度: {similarity:.2f}"],
                    reasoning_path=[
                        f"案例检索: {case['title']}",
    ...
    async def _hybrid_reasoning(
        self, initial_results: list[ReasoningResult], query: str, context: str,
    ) -> list[ReasoningResult]:
        """混合推理"""
        results = []
        ...
        try:
            # 找到置信度最高的几个结果
            top_results = sorted(initial_results, key=lambda x: x.confidence, reverse=True)[:3]

            # 构建混合推理结论
            conclusions = [result.conclusion for result in top_results]
            confidences = [result.confidence for result in top_results]
            ...
            for i, result in enumerate(top_results, 1):
                reasoning_path.append(
                    f"{i}. {result.reasoning_type.value}: {result.confidence:.2f}"
                )
    ...
    async def add_custom_rule(self, rule: ReasoningRule):
        ...
        logger.info(f"✅ 添加自定义推理规则: {rule.name}")

    async def add_case(self, case_id: str, case_data: dict[str, Any]):
        """添加案例"""
        self.case_database[case_id] = case_data
        logger.info(f"✅ 添加案例: {case_id}")
    ...
    async def export_reasoning_knowledge(self, file_path: str):
        """导出推理知识库"""
        try:
            export_data = {
                ...
                "statistics": await self.get_reasoning_statistics(),
            }

            # 转换推理规则为可序列化格式
            for rule_id, rule in self.reasoning_rules.items():
                export_data.get("reasoning_rules")[rule_id] = {
            ...
            logger.info(f"✅ 推理知识库已导出到: {file_path}")
...
async def main():
    ...
    try:
                ...
                confidence_level = (
                    ...
                    if result.confidence > 0.9
                        ...
                        if result.confidence > 0.7
                            ...
                            else "低" if result.confidence > 0.3 else "非常低"
                        )
                    )
                )

                print(
                    f"     {j}. [{result.reasoning_type.value}] 置信度: {result.confidence:.2f} ({confidence_level})"
                )
                print(f"        结论: {result.conclusion:80}...")
                print(f"        推理路径: {' → '.join(result.reasoning_path[:2])}")
        ...
        # 导出知识库
        export_file = f"/tmp/reasoning_knowledge_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"  # TODO: 确保除数不为零
        await engine.export_reasoning_knowledge(export_file)
        print(f"   知识库导出: {export_file}")
