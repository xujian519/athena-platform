...
class OnDemandAgentOrchestrator:
    ...
    def _load_agent_configs(self) -> dict[AgentType, AgentConfig]:
        ...
        configs = {
            ...
            AgentType.YUNXI: AgentConfig(
                ...
                port=8087,
                script_path="services/yunpat-agent/app/main.py",
                startup_timeout=30,
                idle_timeout=300,  # 5分钟空闲后停止
                auto_stop=True,
    ...
    async def _determine_target_agent(self, task_request: TaskRequest) -> AgentType | None:
        """确定目标智能体"""
    ...
    async def _wait_for_agent_ready(self, instance: AgentInstance, timeout: int):
        ...
        raise asyncio.TimeoutError(f"智能体启动超时 ({timeout}秒)")
