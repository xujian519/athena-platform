...
class PatentDBPoolManager:
    ...
    async def initialize(self, db_config: dict[str, Any] | None = None):
        """初始化连接池"""
        if self.initialized:
            return
    ...
    async def batch_insert(
        self, table: str, data: list[dict[str, Any]], batch_size: int | None = None
    ) -> int:
        """批量插入(优化版)"""
        if not data:
            return 0
...
# 全局单例
_patent_pool_manager: PatentDBPoolManager | None = None
