...
class PatentRulesBuilder:
    ...
    def __init__(self):
        ...
        self.vector_store_path = self.output_path / "vector_db"
        self.graph_path = self.output_path / "knowledge_graph"
    ...
    async def process_document(self, doc_info: dict) -> list[VectorDocument | None]:
        ...
        try:
            ...
            # 使用法律数据处理器
            content_result = await self.legal_processor.process_document(file_path)
            ...
            # 分段处理
            documents = []
            sections = self.legal_processor.split_legal_document(content)
    ...
    async def extract_entities_and_relations(self, documents: list[VectorDocument]) -> dict:
        ...
        for doc in documents:
            try:
                # 提取实体和关系
                result = await self.extractor.extract_entities_and_relations(
                    doc.content,
                    doc.doc_id
                )
                ...
                for relation in result.get('relations', []):
                    relation['source_document'] = doc.doc_id
    ...
    async def build_vector_database(self, all_documents: list[VectorDocument]):
        ...
        try:
            ...
            for i in range(0, len(all_documents), batch_size):
                ...
                # 批量索引
                await self.vector_store.batch_index(batch, collection_name)
    ...
    async def build_knowledge_graph(self, entities_and_relations: dict):
        ...
        try:
            ...
            # 构建子图
            if entities:
                await self.graph_builder.build_subgraph([e['name'] for e in entities[:10]])
                logger.info("✅ 构建示例子图")
    ...
    async def generate_report(self):
        ...
        report = {
            ...
            "输出位置": {
                "向量数据库": str(self.vector_store_path),
                "知识图谱": str(self.graph_path)
            }
        }
