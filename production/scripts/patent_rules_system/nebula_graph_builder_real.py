#!/usr/bin/env python3
"""
NebulaGraph知识图谱构建器 - 真实版本
NebulaGraph Knowledge Graph Builder - Real Version
...
class NebulaGraphBuilderReal:
    ...
    async def connect(self):
        ...
        try:
            ...
            await self.pool.init([(host, port) for host, port in self.addresses], self.config)
    ...
    async def initialize_space(self):
        ...
        try:
            # 检查图空间是否存在
            check_space_query = f"SHOW SPACES LIKE '{self.space_name}'"
            result = await self.execute_query(check_space_query)
            ...
            if result.row_size() == 0:
                ...
                result = await self.execute_query(create_space_query)
                if not result.is_succeeded():
            ...
            result = await self.execute_query(use_space_query)
            if not result.is_succeeded():
    ...
    async def _create_edge_types(self):
        """创建边类型"""
        edges = {
        ...
        for etype, entity_list in entities_by_type.items():
            ...
            for entity in entity_list[:100]:  # 限制批量大小
                ...
                if 'source' in properties:
                    source = properties['source'].replace('"', '\\"')
                    prop_strs.append(f'source: "{source}"')
                if 'document_type' in properties:
                    doc_type = properties['document_type'].replace('"', '\\"')
            ...
            if values:
                query = f"""
                INSERT VERTEX {etype} ({', '.join([f'{p.split(":")[0]}' for p in prop_strs.split(", ")] if "prop_strs" in locals() else "")})
                VALUES {', '.join(values)}
                """.replace("  ", "")
                ...
                if result.is_succeeded():
                    ...
                    entity_ids.extend([entity.get('name', entity.get('text')) for entity in entity_list])
                else:
        ...
        for rtype, relation_list in relations_by_type.items():
            ...
            for relation in relation_list[:100]:  # 限制批量大小
                ...
                if 'relation_type' in properties:
                    rel_type = properties['relation_type'].replace('"', '\\"')
            ...
            if values:
                # 查询属性名称
                prop_names = []
                if "prop_strs" in locals() and prop_strs:
                    prop_names = [p.split(":")[0] for p in prop_strs.split(", ")]

                query = f"""
                INSERT EDGE {rtype} ({', '.join(prop_names)})
                VALUES {', '.join(values)}
                """.replace("  ", "")
                ...
                if result.is_succeeded():
                    ...
                    relation_ids.extend([f"{r.get('subject')}-{r.get('object')}" for r in relation_list])
                else:
                    logger.warning(f"  ⚠️ 插入 {rtype} 关系失败: {result.error_msg()[:100]}")

        return relation_ids

    async def query_entity(self, entity_name: str) -> dict[str, Any | None]:
        """查询实体"""
    ...
    async def query_relations(self, entity_name: str, relation_type: str = None) -> list[dict[str, Any]]:
        """查询关系"""
        if relation_type:
...
async def test_nebula_connection():
    """测试NebulaGraph连接"""
    builder = NebulaGraphBuilderReal()

    try:
        # 初始化图空间
        success = await builder.initialize_space()
        if success:
            ...
            entity_ids = await builder.add_entities(test_entities)
            ...
            relation_ids = await builder.add_relations(test_relations)
            print(f"✅ 添加测试关系: {len(relation_ids)} 个")

            # 查询测试
            entity = await builder.query_entity('发明专利')
            if entity:
                print(f"✅ 查询实体成功: {entity}")

            relations = await builder.query_relations('专利法')
...
if __name__ == "__main__":
    ...
    asyncio.run(test_nebula_connection())
