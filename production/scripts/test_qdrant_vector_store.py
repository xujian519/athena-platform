...
"""
测试Qdrant向量库系统
Test Qdrant Vector Store System
...
async def test_collection_creation():
    ...
    from patent_rules_system.qdrant_vector_store import QdrantVectorStore

    store = QdrantVectorStore(collection_name="test_patent_rules")
    ...
    config_file = data_dir / "test_patent_rules_config.json"
...
async def test_document_indexing():
    ...
    from patent_rules_system.qdrant_vector_store import (
        DocumentType,
        QdrantVectorStore,
        VectorDocument,
    )
    ...
    # 检查数据文件
    data_file = Path("/Users/xujian/Athena工作平台/production/data/patent_rules/qdrant_store/test_indexing_data.json")
    if data_file.exists():
...
async def test_search_modes():
    ...
    store = QdrantVectorStore(collection_name="test_search")
    await store.create_collection()
...
async def test_rerank_optimization():
    ...
    store = QdrantVectorStore(collection_name="test_rerank")
    await store.create_collection()
...
async def test_batch_operations():
    ...
    store = QdrantVectorStore(collection_name="test_batch")
    await store.create_collection()
...
async def test_statistics():
    ...
    store = QdrantVectorStore(collection_name="test_stats")
...
async def main():
    ...
    # 保存测试报告
    report_file = Path("/Users/xujian/Athena工作平台/production/logs/qdrant_vector_store_test_report.json")
