...
# 导入智能决定要点提取器
from production.scripts.review_decision.smart_keypoints_extractor import (
    SmartKeypointsExtractor,
)
...
logging.basicConfig(
    ...
    handlers=[
        logging.file_handler(f'/Users/xujian/Athena工作平台/logs/docx_pipeline_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class DocxOnlyPipeline:
    ...
    async def process_batch(self, file_paths: list[Path], batch_index: int, total_batches: int):
        ...
        logger.info(f"  完成: {len(all_chunks)}块, {len(vectors)}向量 | 速度: {speed:.1f}文件/秒")
    ...
    async def run_full_pipeline(self, batch_size: int = 2000):
        ...
        logger.info("📄 DOC文件数: 20,503 (暂不处理，需要单独转换)")
        logger.info("")
        ...
        logger.info("=" * 70)
        logger.info("🎉 验证批次处理完成！")
        logger.info("=" * 70)
        logger.info(f"📊 本次处理: {self.stats['files_processed']} 文件 (验证批次)")
        ...
        logger.info("")
        logger.info(f"📝 剩余DOCX: {len(docx_files) - self.stats['files_processed']:,} 个")
        logger.info("📄 待处理DOC: 20,503 个")
        logger.info("")
        logger.info("💡 请验证数据质量后，再继续处理剩余文件")
