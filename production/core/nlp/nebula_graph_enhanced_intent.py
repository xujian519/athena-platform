...
原功能说明:
NebulaGraph增强的意图识别系统
NebulaGraph Enhanced Intent Recognition System
利用知识图谱提升意图识别准确率
"""
...
try:
    ...
    print("⚠️ NebulaGraph未安装,将使用模拟模式")
    ConnectionPool = None
    Config = None
...
class NebulaGraphConnection:
    ...
    async def connect(self) -> bool:
        ...
        try:
            if ConnectionPool is None:
                print("⚠️ NebulaGraph未安装,使用模拟模式")
                self.mock_mode = True
                self.connected = True
                return True

            self.connection_pool = ConnectionPool()
            if self.connection_pool.connect([(self.hosts[0], self.config)]):
                self.connected = True
    ...
    async def search_intent_entities(self, text: str) -> dict:
        ...
        try:
            ...
            for keyword in keywords:
                ...
                LIMIT 5
                """
                patent_results = await self._execute_query(patent_query)
                results["patent_entities"] = patent_results

                # 查询法律实体
                legal_query = f"""
                MATCH (v:legal_entity)
                WHERE v.name CONTAINS "{keyword}"
                RETURN v.name AS name, v.type AS type
                LIMIT 5
                """
                legal_results = await self._execute_query(legal_query)
                results["legal_entities"] = legal_results
    ...
    async def _execute_query(self, query: str):
        ...
        # 模拟实现
        mock_results = [
            {"name": f"专利实体{i}", "type": "patent_entity"} for i in range(random.randint(0, 3))
        ]
        return mock_results
    ...
    def _extract_keywords(self, text: str) -> list[str]:
        ...
        # 过滤关键词
        keywords = [
            w for w in words if len(w) > 1 and w not in ["的", "是", "在", "有", "和", "与", "或"]
        ]
        return keywords[:10]
...
class GraphEnhancedIntentClassifier:
    ...
    def _calculate_graph_contribution(self, graph_scores):
        ...
        if max_score > min_score:
            ...
            return {"enhancement_level": "low", "max_score": max_score, "entity_count": 0}

    def _build_reasoning(self, text: str, intent: str, entities: dict):
        """构建推理链"""
        reasoning = [f"1️⃣ 文本分析: '{text}'", f"2️⃣ 图谱检索: 找到{len(entities)}类实体"]
        ...
        reasoning.extend(["3️⃣ 特征融合: 文本特征 + 图谱特征", f"4️⃣ 最终判定: {intent}"])
