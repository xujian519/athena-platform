...
# 导入审查意见解析器
try:
    from core.patent.oa_document_parser import (
        OfficeActionParser,
        ParsedOfficeAction,
        RejectionType,
    )
...
class HumanInteractionWorkflow:
    ...
    def _init_new_modules(self):
        ...
        # 文档完整性检查器
        try:
            from core.patent.document_completeness_checker import (
        ...
        # 专利下载管理器
        try:
            from core.patent.patent_download_manager import get_patent_download_manager
        ...
        # 分析计划生成器
        try:
            from core.patent.analysis_plan_generator import get_analysis_plan_generator
    ...
    async def _step_intelligent_analysis(self) -> dict[str, Any]:
        ...
        # 初始化步骤2执行器
        try:
            from core.patent.step2_analysis_executor import (
