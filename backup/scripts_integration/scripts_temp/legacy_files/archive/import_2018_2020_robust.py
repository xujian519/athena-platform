#!/usr/bin/env python3
"""
2018-2020年专利数据导入脚本 - 健壮版本
基于成功的test_single_2018.py，确保数据一致性
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2018_2020_robust.log'),
...
def import_year_data(year, batch_size=1000, max_records=10000):
    ...
    try:
        ...
        with open(file_path, 'r', encoding='utf-8-sig') as csvfile:  # 使用utf-8-sig处理BOM
            ...
            logger.info(f"SQL占位符数量: {sql.count('%s')}")
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                try:
                    ...
                    # 确保有36个字段
                    if len(values) != 36:
                        logger.error(f"数据字段数不正确: {len(values)} (第{row_num}行)")
                        skipped += 1
                        continue
...
def main():
    """主函数"""
    logger.info('=' * 80)
    logger.info('开始导入2018-2020年专利数据（健壮版本）')
    logger.info('=' * 80)
