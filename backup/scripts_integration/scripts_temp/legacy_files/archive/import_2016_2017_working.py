...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2016_2017_working.log'),
...
def import_year_data(year, batch_size=100, max_records=1000):
    """导入一年的专利数据"""
...
def main():
    """主函数"""
    logger.info('=' * 80)
    logger.info('开始导入2016-2017年专利数据（工作版本）')
    logger.info('=' * 80)
    ...
    for year in years:
        logger.info(f"\n开始处理 {year} 年数据...")
        # 测试导入100条
        success = import_year_data(year, batch_size=10, max_records=100)
