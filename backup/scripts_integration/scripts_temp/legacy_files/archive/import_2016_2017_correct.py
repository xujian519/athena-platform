...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2016_2017_correct.log'),
        ...
        with open(file_path, 'r', encoding='utf-8-sig') as csvfile:  # 使用utf-8-sig处理BOM
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                try:
                    ...
                    # 引证次数相关字段（虽然用户说不需要，但为了完整性保留）
                    citation_count = float(row.get('引证次数', 0) or 0)
                    cited_count = float(row.get('被引证次数', 0) or 0)
                    self_citation_count = float(row.get('自引次数', 0) or 0)
                    other_citation_count = float(row.get('他引次数', 0) or 0)
                    cited_by_self_count = float(row.get('被自引次数', 0) or 0)
                    cited_by_others_count = float(row.get('被他引次数', 0) or 0)
                    family_citation_count = float(row.get('家族引证次数', 0) or 0)
                    family_cited_count = float(row.get('家族被引证次数', 0) or 0)
...
def main():
    """主函数"""
    logger.info('=' * 80)
    logger.info('开始导入2016-2017年专利数据（正确版本）')
    logger.info('=' * 80)
