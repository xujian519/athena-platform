#!/usr/bin/env python3
"""
最终版：导入2021年中国专利数据到PostgreSQL数据库
解决所有已知问题：BOM、字段数、数据格式
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2021_final.log'),
...
def check_and_prepare_database(conn):
    """检查并准备数据库"""
    cursor = conn.cursor()
...
def import_2021_patents():
    ...
    try:
        # 准备数据库
        check_and_prepare_database(conn)
        cursor = conn.cursor()
