...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2019_repaired.log'),
        ...
        with open(file_path, 'r', encoding='utf-8-sig') as csvfile:
            ...
            for row_num, row in enumerate(reader, 1):
                ...
                try:
                    ...
                    # 引证次数相关字段
                    citation_count = safe_int(row.get('引证次数', 0))
                    cited_count = safe_int(row.get('被引证次数', 0))
                    self_citation_count = safe_int(row.get('自引次数', 0))
                    other_citation_count = safe_int(row.get('他引次数', 0))
                    cited_by_self_count = safe_int(row.get('被自引次数', 0))
                    cited_by_others_count = safe_int(row.get('被他引次数', 0))
                    family_citation_count = safe_int(row.get('家族引证次数', 0))
                    family_cited_count = safe_int(row.get('家族被引证次数', 0))
...
def main():
    """主函数"""
    logger.info('=' * 80)
    logger.info('开始导入2019年专利数据（修复版本）')
    logger.info('=' * 80)
    ...
    try:
        ...
        if conn:
            cursor = conn.cursor()
            cursor.execute('SELECT COUNT(*) FROM patents WHERE source_year = 2019')
            existing_count = cursor.fetchone()[0]
            cursor.close()
            conn.close()
            logger.info(f"数据库中已有 2019 年数据: {existing_count:,} 条")
    except Exception as e:
        logger.warning(f"无法查询现有数据: {e}")

    # 导入2019年数据
    success = import_year_data(2019, batch_size=2000, start_from=0)

    if not success:
        logger.error('❌ 2019年数据导入失败')
        return False

    logger.info('✅ 2019年数据导入完成！')
    return True
