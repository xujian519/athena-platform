...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2016_2017_repaired.log'),
...
def import_year_data(year, batch_size=1000, max_records=None):
    """导入一年的专利数据"""
...
def main():
    """主函数"""
    logger.info('=' * 80)
    logger.info('开始导入2016-2017年专利数据（修复版本）')
    logger.info('=' * 80)
    ...
    for year in years:
        ...
        # 导入完整数据
        success = import_year_data(year, batch_size=2000, max_records=None)
    ...
    logger.info('✅ 2016-2017年所有数据导入完成！')
    return True
