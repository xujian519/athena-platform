#!/usr/bin/env python3
"""
成功版：导入2021年中国专利数据到PostgreSQL数据库
包含正确的专利类型映射和字段处理
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('import_2021_success.log'),
...
def normalize_patent_type(patent_type):
    ...
    # 发明类型映射
    if patent_type in ['发明授权', '发明申请', '发明']:
        return '发明'

    # 实用新型类型映射
    if patent_type in ['实用新型授权', '实用新型申请', '实用新型']:
        return '实用新型'

    # 外观设计类型映射
    if patent_type in ['外观设计授权', '外观设计申请', '外观设计']:
        return '外观设计'
...
def import_2021_patents():
    ...
    try:
        ...
        with open(csv_file, 'r', encoding='utf-8-sig') as f:
            ...
            for row_num, row in enumerate(reader, 2):
                try:
                    ...
                    # 标准化专利类型
                    normalized_type = normalize_patent_type(row[1])
                    if not normalized_type:
                        type_issues += 1
                        if type_issues <= 5:  # 只显示前5个警告
                            logger.warning(f"第{row_num}行: 无效专利类型 '{row[1]}'，跳过")
                        continue

                    # 处理数据
                    patent_data = [
                        row[0].strip() if row[0] else '',   # patent_name
                        normalized_type,                    # patent_type (标准化)
        ...
        logger.info(f"⚠️ 类型问题: {type_issues:,} 条")
