...
class PatentSearchRequest(BaseModel):
    ...
    filters: dict[str, Any] | None = Field(default=None, description='搜索筛选器')
...
class TaskListRequest(BaseModel):
    status: str | None = Field(default=None, pattern='^(pending|running|completed|failed|cancelled)$', description='任务状态')
...
async def search_patents(request: PatentSearchRequest):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"不支持的数据源: {request.source}") from None

        # 转换优先级
        try:
            priority = PatentPriority(request.priority)
        except ValueError:
            raise HTTPException(status_code=400, detail=f"无效的优先级: {request.priority}") from None
    ...
    try:
        ...
        if request.status:
            try:
                ...
                raise HTTPException(status_code=400, detail=f"无效的状态: {request.status}") from None
