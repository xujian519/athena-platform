...
def show_processing_summary():
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info("✅ Neo4j Web界面可访问: http://localhost:7474")
    except Exception:
        logger.info("⚠️ Neo4j Web界面不可访问")

    logger.info("\n💡 有用的查询:")
    logger.info("1. 查看节点总数: MATCH (n) RETURN count(n)")
    logger.info("2. 查看节点类型: MATCH (n) RETURN labels(n)[0], count(n)")
    logger.info("3. 查看关系总数: MATCH ()-[r]->() RETURN count(r)")
