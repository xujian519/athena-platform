...
try:
    from .patent_tools import InvalidityPredictor, LegalKnowledgeEnhancer, PatentInvalidator
    PATENT_TOOLS_AVAILABLE = True
except ImportError:
    PATENT_TOOLS_AVAILABLE = False

try:
    from .data_tools import DataAnalyzer, DataCleaner, DataProcessor
    DATA_TOOLS_AVAILABLE = True
except ImportError:
    DATA_TOOLS_AVAILABLE = False

try:
    from .analysis_tools import MLAnalyzer, StatisticalAnalyzer, TextAnalyzer
    ANALYSIS_TOOLS_AVAILABLE = True
except ImportError:
    ANALYSIS_TOOLS_AVAILABLE = False

try:
    from .report_tools import ChartGenerator, DocumentProcessor, ReportGenerator
