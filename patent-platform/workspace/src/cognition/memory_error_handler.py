...
class ErrorHandler:
    ...
    def _log_error(self, error: MemorySystemError):
        ...
        if error.severity == ErrorSeverity.CRITICAL:
            self.logger.critical(f"🚨 {error.message}: {error_dict}")
        elif error.severity == ErrorSeverity.HIGH:
            self.logger.error(f"❌ {error.message}: {error_dict}")
        elif error.severity == ErrorSeverity.MEDIUM:
            self.logger.warning(f"⚠️ {error.message}: {error_dict}")
        else:
            self.logger.info(f"ℹ️ {error.message}: {error_dict}")
