...
class MultimodalContent:
    ...
    raw_data: str | bytes | np.ndarray = None
...
class ImageProcessor:
    ...
    def extract_text_from_image(self, image: str | np.ndarray) -> dict[str, Any]:
        """从图像中提取文本"""
        try:
    ...
    def detect_layout_elements(self, image: str | np.ndarray) -> list[dict[str, Any]]:
        """检测图像中的布局元素"""
        try:
            if isinstance(image, str):
                img = cv2.imread(image)
    ...
    def analyze_diagram_structure(self, image: str | np.ndarray) -> dict[str, Any]:
        """分析图表结构"""
        try:
            if isinstance(image, str):
                img = cv2.imread(image)
...
class TableExtractor:
    ...
    def extract_table_from_image(self, image: str | np.ndarray) -> dict[str, Any]:
        """从图像中提取表格"""
        try:
            if isinstance(image, str):
                img = cv2.imread(image)
            ...
            if cells:
                # 确定行列数
                x_positions = sorted({c['bounding_box']['x'] for c in cells})
                y_positions = sorted({c['bounding_box']['y'] for c in cells})
