...
class RandomForestDecisionModel(DecisionModel):
    ...
    def _prepare_training_data(self, data: list[dict[str, Any]]) -> tuple[Any, Any]:
        ...
        # 去重并排序特征名
        self.feature_names = sorted(set(self.feature_names))
...
class EnsembleVotingModel(DecisionModel):
    ...
    def __init__(self, models: list[DecisionModel] | None = None, voting: str = 'soft'):
    ...
    async def train(self, data: list[dict[str, Any]]) -> float:
        ...
        if accuracies and self.model_weights:
            self.accuracy = sum(a * w for a, w in zip(accuracies, self.model_weights, strict=False)) / sum(self.model_weights)
        else:
            self.accuracy = 0.0
    ...
    async def predict(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.voting == 'soft':
            ...
            for pred, weight in zip(predictions, self.model_weights, strict=False):
