...
class MemoryProcessor:
    ...
    async def retrieve_memory(self, cue: RetrievalCue) -> RetrievalResult:
        ...
        # 排序并限制结果数量
        sorted_results = sorted(zip(matched_traces, relevance_scores, strict=False),
                               key=lambda x: x[1],
...
async def main():
    ...
    for i, (trace, score) in enumerate(zip(result.matched_traces, result.relevance_scores, strict=False), 1):
        logger.info(f"\n记忆 {i}:")
