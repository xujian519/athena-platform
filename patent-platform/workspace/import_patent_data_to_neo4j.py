...
class PatentNeo4jImporter:
    ...
    def import_patent_document(self, patent_data: Dict, session: Session) -> Dict[str, int]:
        ...
        try:
            ...
            patent_node = self._create_patent_node(session, patent_data)
            stats['nodes'] += 1
            ...
            for claim in claims:
                claim_node = self._create_claim_node(session, claim, patent_id)
                stats['nodes'] += 1
...
def main():
    ...
    try:
        ...
        logger.info(f"🔍 Bolt连接: bolt://localhost:7687")
