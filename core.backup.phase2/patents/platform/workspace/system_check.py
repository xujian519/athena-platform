...
def check_python_version():
    ...
    if sys.version_info >= (3, 8):
        ...
        print_status(f"Python版本: {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} (需要3.8+)', 'ERROR")
        return False
...
def main():
    ...
    if success:
        ...
        logger.info(f"\n💡 下一步建议:")
        logger.info(f"   1. 运行小批量测试: python3 test_small_batch.py")
        logger.info(f"   2. 启动Web界面: python3 build_patent_kg.py web")
        logger.info(f"   3. 开始构建知识图谱")
    else:
