...
class PatentDraftingProxy(BaseXiaonaComponent):
    ...
    def _get_unified_writer(self) -> Any:
        """
    ...
    def _build_adapter_context(self, task_id: str, data: dict[str, Any]) -> AgentExecutionContext:
        """
        构建适配器执行上下文（私有辅助方法）
...
def create_patent_drafting_proxy(
    agent_id: str = "patent_drafting_proxy", config: Optional[dict[str, Any]] = None
) -> PatentDraftingProxy:
    """
    创建专利撰写代理实例（向后兼容工厂函数）
...
def get_patent_drafting_proxy() -> PatentDraftingProxy:
    """
