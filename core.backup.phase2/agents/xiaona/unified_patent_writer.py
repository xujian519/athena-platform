...
"""
    ...
    async def draft_full_application(
        self, disclosure_data: Dict[str, Any], progress_callback=None
    ) -> Dict[str, Any]:
        """
        完整专利申请流程（便捷方法）
    ...
    async def draft_office_action_response(
        self, office_action: Dict[str, Any], progress_callback=None
    ) -> Dict[str, Any]:
        """
        审查意见答复流程（便捷方法）
...
def create_unified_patent_writer(config: Optional[Dict[str, Any]] = None) -> UnifiedPatentWriter:
    """
    创建统一专利撰写代理实例
...
def get_unified_patent_writer() -> UnifiedPatentWriter:
    """
    获取单例统一专利撰写代理实例
