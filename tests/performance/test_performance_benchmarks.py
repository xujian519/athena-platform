...
from tests.integration.xiaochen_collaboration_integration import XiaochenEnhancedAgent
from tests.integration.xiaona_chain_integration import XiaonaEnhancedAgent
from tests.integration.yunxi_goal_integration import YunxiEnhancedAgent
...
class PerformanceBenchmark:
    ...
    def __init__(self):
        ...
        self.baseline_file = Path(
            "/Users/xujian/Athena工作平台/tests/data/performance_baseline.json"
        )
    ...
    def compare_with_baseline(
        self, component: str, operation: str, actual_value: float
    ) -> dict[str, Any]:
        """与基准比较"""
        ...
        if "max_time" in baseline_data:
            if actual_value <= baseline_data["max_time"]:
                return {
                    "status": "passed",
                    "message": f"性能符合基准 (≤{baseline_data['max_time']}s)",
                }
            else:
                return {
                    "status": "failed",
...
class TaskPlannerPerformanceTest(unittest.TestCase):
    ...
    def test_create_plan_performance(self):
        ...
        for task_name, goal, context in test_cases:
            ...
            self.benchmark.record_metric(
                "task_planner", "create_plan_memory", memory_increase, "MB"
            )

            # 与基准比较
            comparison = self.benchmark.compare_with_baseline(
                "task_planner", "create_plan", execution_time
            )
    ...
    def test_concurrent_planning_performance(self):
        ...
        async def test_concurrent():
            ...
            tasks = [
                asyncio.to_thread(self.planner.create_execution_plan, goal, {}) for goal in goals
            ]
            ...
            print(
                f"🔄 并发规划: {successful_plans}/{concurrent_count} 成功, 平均 {avg_time:.3f}s/任务"
            )
...
class PromptChainPerformanceTest(unittest.TestCase):
    ...
    def test_chain_creation_performance(self):
        ...
        for result in results:
            if result["success"]:
                comparison = self.benchmark.compare_with_baseline(
                    "prompt_chain", "create_chain", result["execution_time"]
                )
...
class GoalManagerPerformanceTest(unittest.TestCase):
    ...
    def test_goal_creation_performance(self):
        ...
        for i, template in enumerate(goal_templates):
            ...
            print(
                f"🎯 目标{i + 1}创建: {execution_time:.3f}s, {len(goal.subgoals) if goal else 0}个子目标"
            )

        # 验证性能指标
        for result in results:
            comparison = self.benchmark.compare_with_baseline(
                "goal_manager", "create_goal", result["execution_time"]
            )
            if comparison["status"] == "failed":
    ...
    def test_progress_update_performance(self):
        ...
        goal = self.goal_manager.create_goal(
            {"title": "性能测试目标", "description": "用于测试进度更新性能", "priority": 2}
        )
        ...
        for result in results:
            comparison = self.benchmark.compare_with_baseline(
                "goal_manager", "update_progress", result["avg_time"]
            )
            if comparison["status"] == "failed":
...
class AgentIntegrationPerformanceTest(unittest.TestCase):
    ...
    async def test_agent_response_time_performance(self):
        """测试智能体响应时间性能"""
        test_requests = {
            "xiaonuo": ["制定简单的学习计划", "分析系统性能问题", "设计复杂的技术架构"],
            "xiaona": ["搜索AI相关专利", "分析技术专利布局", "评估专利侵权风险"],
            "yunxi": ["设定学习Python的目标", "制定健身计划", "创建项目管理目标"],
            "xiaochen": ["协调两人协作任务", "组织团队项目", "管理复杂的多智能体项目"],
        }
        ...
        for result in results:
            comparison = self.benchmark.compare_with_baseline(
                "agent_integrations", "response_time", result["avg_response_time"]
            )
            if comparison["status"] == "failed":
            ...
            self.assertGreaterEqual(
                result["success_rate"], 0.9, f"智能体成功率过低: {result['agent']}"
            )
    ...
    async def test_concurrent_agent_performance(self):
        ...
        # 验证并发性能
        comparison = self.benchmark.compare_with_baseline(
            "agent_integrations", "concurrent_load", avg_time_per_request
        )
        if comparison["status"] == "failed":
    ...
    def test_large_dataset_memory_usage(self):
        ...
        for i in range(large_goal_count):
            goal_data = {
                ...
                "priority": (i % 4) + 1,
                "metrics": [
                    {"name": f"指标{j}", "target": j * 10, "unit": "单位"} for j in range(5)
                ],
            }
