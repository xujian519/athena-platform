...
func NewAdaptiveRateLimiter(cfg *config.RateLimiterConfig) (*AdaptiveRateLimiter, error) {
	config := &RateLimiterConfig{
		...
		StatsWindow:      time.Duration(cfg.StatsWindow) * time.Second,
	}
	...
	logging.LogInfo("自适应限流器初始化成功",
		...
		zap.Bool("adaptive_mode", config.AdaptiveMode),
	)
...
func (r *AdaptiveRateLimiter) calculateWaitTime(bucket *TokenBucket) time.Duration {
	...
	return minDuration(waitTime, time.Duration(r.config.Window)*time.Second)
}
...
func (r *AdaptiveRateLimiter) adjustLimits() {
	...
	for key, bucket := range r.limiters {
		...
		// 根据错误率调整
		bucket.mutex.Lock()
		emptyRate := float64(bucket.stats.emptyCount) / float64(max(1, bucket.stats.refillCount))
		bucket.mutex.Unlock()
		...
		if newLimit != oldLimit {
			...
			logging.LogInfo("自适应调整限流参数",
				...
				zap.Float64("empty_rate", emptyRate),
			)
		}
	}
