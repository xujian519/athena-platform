#!/usr/bin/env python3
"""
创建符号链接 - Phase 3 Batch 2
Create Symbolic Links - Phase 3 Batch 2

为向后兼容性创建符号链接

执行方式:
    python3 scripts/create_symlinks_phase3_batch2.py

清理方式:
    python3 scripts/create_symlinks_phase3_batch2.py --remove
"""
...
# 符号链接映射：旧路径 -> 新路径 (Batch 2)
SYMLINK_MAPPINGS = [
    # patent_hybrid_retrieval -> patents/retrieval
    {
        'old': PROJECT_ROOT / 'patent_hybrid_retrieval',
        'new': PROJECT_ROOT / 'patents' / 'retrieval',
        'description': 'patent_hybrid_retrieval -> patents/retrieval'
    },
    # patent-platform -> patents/platform
    {
        'old': PROJECT_ROOT / 'patent-platform',
        'new': PROJECT_ROOT / 'patents' / 'platform',
        'description': 'patent-platform -> patents/platform'
    },
]
